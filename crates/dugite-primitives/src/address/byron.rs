...
mod tests {
    ...
    // Inner:       array(3)[bstr(28)(root), map(0), u8(2)]
    //
    // Key 1: "-0BJDi-gauylk4LptQTgjMeo7kY9lTCbZv12vwOSTZk=" (first genesis entry)
    //   pubkey = fb40490e2fa06aeca59382e9b504e08cc7a8ee463d95309b66fd76bf03924d99
    //   root   = ccdf735b7d5cafe44e65e75a82fd4305fe7712924a8e1196fefdbddf
    //   crc    = 0x9cbc2ffe
    //
    // Key 2: "-0Np4pyTOWF26iXWVIvu6fhz9QupwWRS2hcCaOEYlw0=" (second genesis entry)
    // -----------------------------------------------------------------------

    const GOLDEN_KEY1_HEX: &str =
        "82d818582183581cccdf735b7d5cafe44e65e75a82fd4305fe7712924a8e1196fefdbddfa0021a9cbc2ffe";
    const GOLDEN_KEY2_HEX: &str =
        "82d818582183581cb8c55416d1426377481c149101a55e8540d0a0130e8ea4b344c11254a0021a892590f1";

    /// Base64url-decode the AVVM key and return its raw 32 bytes.
    fn decode_avvm_key(b64: &str) -> [u8; 32] {
        // Normalise URL-safe base64 chars before decoding.
    ...
    /// Build wire-format redeem address bytes from a base64url AVVM key + magic.
    fn avvm_to_address(pubkey_b64: &str, protocol_magic: u32) -> Vec<u8> {
        ...
        ByronAddressPayload::new_redeem(&pubkey, network_tag).to_wire_bytes()
    }
    ...
    #[test]
    fn test_golden_mainnet_key1_byte_exact() {
        let addr_bytes = avvm_to_address("-0BJDi-gauylk4LptQTgjMeo7kY9lTCbZv12vwOSTZk=", 764824073);
        assert_eq!(hex::encode(&addr_bytes), GOLDEN_KEY1_HEX);
    }

    #[test]
    fn test_golden_mainnet_key2_byte_exact() {
        let addr_bytes = avvm_to_address("-0Np4pyTOWF26iXWVIvu6fhz9QupwWRS2hcCaOEYlw0=", 764824073);
        assert_eq!(hex::encode(&addr_bytes), GOLDEN_KEY2_HEX);
    }

    #[test]
    fn test_golden_mainnet_redeem_structure() {
        // Verify structural fields parsed from the key-1 golden vector.
        let addr_bytes = hex::decode(GOLDEN_KEY1_HEX).unwrap();
        let payload =
            ByronAddressPayload::from_wire_bytes(&addr_bytes).expect("golden must round-trip");

        assert_eq!(payload.addr_type, ByronAddrType::Redeem);
        assert_eq!(
            hex::encode(payload.root),
            "ccdf735b7d5cafe44e65e75a82fd4305fe7712924a8e1196fefdbddf"
        );
    ...
    fn test_crc32_mismatch_rejected() {
        // Flip a bit inside the inner CBOR blob (byte 5 of the wire encoding).
        let mut addr_bytes = hex::decode(GOLDEN_KEY1_HEX).unwrap();
        addr_bytes[5] ^= 0x01;
    ...
    fn test_testnet_preview_attributes() {
        // Preview magic = 2 → CBOR u32(2) = 0x02 (1 byte).
        // NetworkTag attribute (key 2): map(1){u8(2): bytes([0x02])} = a1 02 41 02
        ...
        assert_eq!(payload.attributes, vec![0xa1u8, 0x02, 0x41, 0x02]);
    }

    #[test]
    fn test_testnet_network_tag_included() {
        let addr_bytes = avvm_to_address("-0BJDi-gauylk4LptQTgjMeo7kY9lTCbZv12vwOSTZk=", 2);
        let payload = ByronAddressPayload::from_wire_bytes(&addr_bytes).unwrap();
        // Testnet: attrs must be non-empty and start with map(1) = 0xa1
        assert!(payload.attributes.len() > 1);
