...
fn test_reward_expansion_no_i128_overflow() {
    ...
    // No pools → full reward_pot (= expansion − treasury_cut) is undistributed → treasury.
    // Correct Haskell accounting: delta_reserves = expansion, delta_treasury = total_rewards_available.
    // expansion = floor(rho * reserves) = floor(1 * MAX) = MAX, so reserves → 0.
    // The main purpose of this test is verifying no i128 overflow in expansion calc.
    let expansion = Rat::new(1i128, 1i128)
        .mul(&Rat::new(MAX_LOVELACE_SUPPLY as i128, 1))
        .floor_u64();
    assert_eq!(
        state.epochs.reserves.0,
        MAX_LOVELACE_SUPPLY - expansion,
        "Reserves should decrease by full expansion when no pools"
    );
}
...
fn test_reward_expansion_large_rho_numerator() {
    ...
    // No pools → full reward_pot is undistributed → treasury.
    // Correct Haskell accounting: delta_reserves = expansion.
...
fn test_reward_tau_zero_no_treasury_cut() {
    ...
    let mut state = make_reward_test_state(initial_reserves, 3, 1000, 0, 1);
    state.utxo.epoch_fees = Lovelace(0);
    let treasury_before = state.epochs.treasury.0;
    ...
    // But per Haskell applyRUpd: undistributed pool rewards still route to treasury.
    // With no pools, full reward_pot = expansion (since treasury_cut=0) → treasury.
    // So delta_treasury = 0 + expansion = expansion; delta_reserves = expansion.
    let expected_expansion = Rat::new(3, 1000)
        .mul(&Rat::new(initial_reserves as i128, 1))
        .floor_u64();
    assert_eq!(
        state.epochs.treasury.0,
        treasury_before + expected_expansion,
        "Treasury should increase by full expansion (undistributed pool rewards → treasury)"
    );
    assert_eq!(
        state.epochs.reserves.0,
        initial_reserves - expected_expansion,
...
fn test_reward_reserves_decrease_treasury_increase() {
    ...
    // Total rewards available = expansion + fees.
    let total_rewards = expected_expansion + 50_000_000;

    // With no pools, full reward_pot (expansion + fees - treasury_cut) is undistributed.
    // Per correct Haskell applyRUpd accounting:
    //   delta_reserves = expansion (fees come from circulating supply, not reserves)
    //   delta_treasury = expansion + fees = total_rewards_available
    // The undistributed reward_pot routes to treasury, not back to reserves.
    let expected_delta_reserves = expected_expansion;
    ...
    assert_eq!(
        ...
        "Reserves should decrease by expansion only (fees come from circulation, not reserves)"
    );

    // Treasury should increase by full total_rewards (tau cut + undistributed reward_pot).
    assert_eq!(
        state.epochs.treasury.0,
        treasury_before + total_rewards,
        "Treasury should increase by full total_rewards when no pools (tau cut + undistributed)"
    );
}
...
fn test_reward_cross_validation_epoch_1239() {
    ...
    //   Correct Haskell applyRUpd accounting: undistributed rewards → treasury.
    //   delta_reserves  = expansion (fees come from circulation, not reserves)
    //   delta_treasury  = total_rewards_available = expansion + fees
    let rupd = state.calculate_rewards(&go_snapshot);

    // Verify delta_reserves equals expansion (only reserves-sourced expansion).
    // Fees came from UTxOs (circulating supply), so they do NOT affect reserves.
    assert_eq!(
        rupd.delta_reserves, expected_expansion,
        "delta_reserves (no-pool case) must equal expansion: \
         expected={expected_expansion}, got={}",
        rupd.delta_reserves
    );

    // Verify delta_treasury equals total_rewards_available (tau cut + full undistributed reward_pot).
    assert_eq!(
        rupd.delta_treasury, total_rewards_available,
        "delta_treasury (no-pool case) must equal total_rewards_available (expansion + fees): \
         expected={total_rewards_available}, got={}",
        rupd.delta_treasury
    );
    ...
    // against the Koios delta_reserves. With active pools on the real chain,
    // Koios reports the true chain delta_reserves which equals expansion.
    // Our no-pool test uses expected_expansion which is the expansion component.
...
fn test_epoch_fees_not_double_counted_through_snapshot_chain() {
    ...
    // expansion = rho * reserves = 3/1000 * 8e15 = 24e12 lovelace.
    let initial_expansion = (3u128 * initial_reserves as u128 / 1000) as u64;
    // Correct Haskell accounting: with no pools in GO, the full reward_pot
    // (= expansion − treasury_cut) is undistributed and goes to treasury.
    // So delta_treasury = treasury_cut + reward_pot = expansion (total_rewards_available).
    // delta_reserves = expansion.
    let expected_treasury_01 = initial_expansion;
    assert_eq!(
        state.epochs.treasury.0, expected_treasury_01,
        "Treasury after 0→1 should equal full expansion (tau cut + full undistributed reward_pot, no pools)"
    );
    let reserves_after_01 = initial_reserves - initial_expansion;
    ...
    let expansion_12 = (3u128 * reserves_after_01 as u128 * effective_blocks as u128)
        / (1000u128 * expected_blocks as u128);
    let total_available_12 = expansion_12 as u64 + epoch0_fees;
    // No pools → full reward_pot is undistributed and goes to treasury.
    // delta_treasury = total_rewards_available = expansion + epoch0_fees.
    let expected_delta_12 = total_available_12;

    let treasury_after_epoch2 = state.epochs.treasury.0;
    let expected_treasury_after_epoch2 = expected_treasury_01 + expected_delta_12;
    let diff =
    ...
    assert!(
        ...
        "Treasury after epoch 1→2 should be ~{expected_treasury_after_epoch2} \
         (full expansion + epoch0 fees, all to treasury since no pools), got {treasury_after_epoch2}. diff={diff}"
    );
    assert!(
        treasury_after_epoch2 > expected_treasury_01,
    ...
    eprintln!(
        "Fee isolation test: epoch0_fees={epoch0_fees}, expected_delta_12={expected_delta_12}, \
         treasury@epoch2={treasury_after_epoch2}, treasury@epoch3={treasury_after_epoch3}, \
...
fn test_rupd_fires_at_first_epoch_canonical_treasury() {
    ...
    // applies it at the 0→1 boundary.  With empty GO (no pools), the full
    // expansion flows to treasury (tau cut + undistributed reward_pot).
    //
    // prev_d=1.0 (genesis) → d >= 0.8 → eta=1 → full expansion.
    // expansion0 = floor(0.003 * 15T) = 45,000,000,000,000
    // delta_treasury = expansion0 (full expansion, no pools to share it)
    // delta_reserves = expansion0
    state.process_epoch_transition(EpochNo(1));

    let expansion0 = 45_000_000_000_000u64; // floor(0.003 * 15T)

    assert_eq!(
        state.epochs.treasury.0, expansion0,
        "Treasury after 0→1 must equal full expansion (no pools: tau cut + undistributed). Got {}",
        state.epochs.treasury.0
    );

    let r0 = 15_000_000_000_000_000u64 - expansion0;
    assert_eq!(
        state.epochs.reserves.0, r0,
        "Reserves after 0→1 must be initial - expansion0: \
         expected={r0}, got={}",
        state.epochs.reserves.0
    );
    ...
    // No pools → delta_treasury1 = expansion1, delta_reserves1 = expansion1
    state.consensus.epoch_block_count = 0;
    state.utxo.epoch_fees = Lovelace(0);
    state.process_epoch_transition(EpochNo(2));
    ...
    assert_eq!(
        ...
        "Treasury after 1→2 must be expansion0({expansion0}) + expansion1({expansion1}), got {}",
        state.epochs.treasury.0
    );
    assert_eq!(
        state.epochs.reserves.0,
        r0 - expansion1,
        "Reserves after 1→2 must be r0({r0}) - expansion1({expansion1}), got {}",
        state.epochs.reserves.0
    );
}
...
///
/// In the no-pool case (correct Haskell applyRUpd):
///   undistributed rewards → treasury (NOT back to reserves).
///   delta_treasury = total_rewards_available = expansion + fees.
///   delta_reserves = expansion.
///
/// Formula (no-pool case, no fees):
///   epoch 0→1: expansion0 = floor(0.003 * R_init);
///              treasury += expansion0; R0 = R_init - expansion0
///   epoch 1→2: expansion1 = floor(0.003 * R0);
///              treasury += expansion1; R1 = R0 - expansion1
///   epoch 2→3: expansion2 = floor(0.003 * R1); etc.
#[test]
fn test_rupd_compounding_treasury_over_three_epochs() {
    ...
    // prev_d=1.0 → d >= 0.8 → eta=1 → full expansion.
    // expansion0 = floor(0.003 * 15T) = 45,000,000,000,000
    // No pools → delta_treasury = expansion0, delta_reserves = expansion0.
    ...
    assert_eq!(
        t0, expansion0,
        "Treasury after 0→1 (initial RUPD, no-pool): expansion0={expansion0}, got {t0}"
    );
    assert_eq!(
        r0,
        initial_reserves - expansion0,
        "Reserves after 0→1 should be initial - expansion0"
    );

    // Epoch 1→2: RUPD fires with bprev=epoch0 blocks (4320), prev_d=0.0.
    // eta = 4320/4320 = 1.0.
    // expansion1 = floor(0.003 * r0)
    // No pools → delta_treasury = expansion1, delta_reserves = expansion1.
    state.consensus.epoch_block_count = full_blocks;
    state.utxo.epoch_fees = Lovelace(0);
    state.consensus.epoch_blocks_by_pool = make_blocks(full_blocks);
    state.process_epoch_transition(EpochNo(2));
    ...
    assert_eq!(
        t1,
        t0 + expansion1,
        "Treasury after 1→2: t0({t0}) + expansion1({expansion1}), got {t1}"
    );
    assert_eq!(
        r1,
        r0 - expansion1,
        "Reserves after 1→2 should be r0({r0}) - expansion1({expansion1})"
    );
    ...
    let exp_ratio_1_2 = expansion1 as f64 / expansion2 as f64;
    let exp_ratio_2_3 = expansion2 as f64 / expansion3 as f64;
    assert!(
        exp_ratio_1_2 > 1.0 && exp_ratio_1_2 < 1.01,
        "Each epoch's expansion should be slightly smaller than the prior: ratio_1_2={exp_ratio_1_2}"
    );
    assert!(
        exp_ratio_2_3 > 1.0 && exp_ratio_2_3 < 1.01,
        "Each epoch's expansion should be slightly smaller than the prior: ratio_2_3={exp_ratio_2_3}"
    );
}
