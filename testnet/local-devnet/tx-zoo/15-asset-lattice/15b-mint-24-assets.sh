#!/usr/bin/env bash
# 15b — mint 24 distinct asset names under one policy.
# 24 is the FIRST size Haskell encodes as an INDEFINITE-length map (0xBF … 0xFF).
# Paired with 15a (23, the last definite one) this brackets the switch that
# #930 got wrong on the encoder side.
set -euo pipefail
...
if zoo_wait_all_observers "$TXID" 120 "$ADDR"; then
    ...
    [ "${GOT:-0}" -ge "$N" ] \
        && zoo_record "$NAME" PASS "$TXID" "minted=$N observed=$GOT indefinite-map-boundary" \
