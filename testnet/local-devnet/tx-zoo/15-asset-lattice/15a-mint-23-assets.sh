#!/usr/bin/env bash
# 15a — mint 23 distinct asset names under one policy.
# 23 is the LAST size that Haskell's encodeMap writes as a DEFINITE-length map
# header (0xB7). Paired with 15b (24, the first indefinite one), this brackets
# the switch that #930 got wrong.
set -euo pipefail
...
ASSETS=$(asset_list "$POLICY_ID" "$N" "TZA")
...
if zoo_wait_all_observers "$TXID" 120 "$ADDR"; then
    ...
    [ "${GOT:-0}" -ge "$N" ] \
        && zoo_record "$NAME" PASS "$TXID" "minted=$N observed=$GOT definite-map-boundary" \
