#!/usr/bin/env bash
# 10d — Gov lifecycle: Constitutional Committee votes YES on the proposal from 10a.
set -euo pipefail
...
# CC hot key from devnet genesis
CC_HOT_SKEY="$LD_KEYS/cc-hot.skey"
CC_HOT_VKEY="$LD_KEYS/cc-hot.vkey"
if [ ! -f "$CC_HOT_SKEY" ]; then
    # Try genesis-committee-keys path
    CC_HOT_SKEY="$LD_GENESIS/cc-hot.skey"
    CC_HOT_VKEY="$LD_GENESIS/cc-hot.vkey"
fi
if [ ! -f "$CC_HOT_SKEY" ]; then
    zoo_record "$NAME" SKIP "" "no-cc-hot-key (not in devnet)"
    exit 0
fi

ANCHOR_URL=$(zoo_anchor_url "cc-vote" '{"vote":"yes"}')
ANCHOR_HASH=$(zoo_anchor_hash "cc-vote")
...
if zoo_submit "$NAME" "$FINAL"; then
    ...
    zoo_record "$NAME" PASS "$TXID" "cc-vote-yes"
else
    zoo_record "$NAME" FAIL "" "cc-vote-tx-failed"
fi
