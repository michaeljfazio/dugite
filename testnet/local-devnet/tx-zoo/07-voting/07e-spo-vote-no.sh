#!/usr/bin/env bash
# 07e — Pool (pool2) votes NO on the InfoAction.
set -euo pipefail
...
zoo_vote_file spo no "$POOL/cold.vkey" "$VOTE" || die "vote-file failed"
...
zoo_wait_all_observers "$TXID" 120 "$ADDR" && zoo_record "$NAME" PASS "$TXID" "spo pool2 NO" \
