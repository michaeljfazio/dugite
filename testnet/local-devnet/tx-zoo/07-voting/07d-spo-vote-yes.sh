#!/usr/bin/env bash
# 07d — Pool (pool1) votes YES on the InfoAction.
set -euo pipefail
...
zoo_vote_file spo yes "$POOL/cold.vkey" "$VOTE" || die "vote-file failed"
...
zoo_wait_all_observers "$TXID" 120 "$ADDR" && zoo_record "$NAME" PASS "$TXID" "spo pool1 YES" \
