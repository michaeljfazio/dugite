#!/usr/bin/env bash
# 03k — Plutus V1 spend with datum-hash + witness-reveal pattern (pre-Babbage).
# Lock with --tx-out-datum-hash, spend with --tx-in-datum-file. Tests the
# datum hash → datum binding inside the witness set.
set -euo pipefail
