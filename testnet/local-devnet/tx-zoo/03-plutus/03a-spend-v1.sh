#!/usr/bin/env bash
# 03a — Plutus V1 spend.
# 1. Lock funds at the always-true V1 address with a datum hash.
# 2. Spend them back, providing the datum + redeemer + collateral.
set -euo pipefail
...
[ -s "$SCRIPT" ] || { zoo_record_env_skip "$NAME" "missing-script-binary $(basename "$SCRIPT") — run lib/build-plutus.sh"; exit 0; }
