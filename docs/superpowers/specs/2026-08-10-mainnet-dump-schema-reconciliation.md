...
3. **conwayGov** — blocked. Needs real Conway oracle output to align against.
   The cheapest source is a fresh preprod Mithril import: Mithril snapshots
   *are* cardano-node's own ImmutableDB chunks and preprod is at PV11, so
   cstreamer can read one directly with no dugite writer involved.
   Then: move `drepDistr` under `conwayGov`, add `committee`, `constitution`,
   `committeeState`, `nextEnactState`, and decide `proposals` / `enactedRoots`
   against `nextEnactState.ensPrevGovActionIds` rather than by assertion.
