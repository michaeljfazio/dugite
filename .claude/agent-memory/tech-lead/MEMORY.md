...
- [#767 Lens A static-cycle analysis (2026-06-16)](issue-767-live-apply-deadlock.md) — exhaustive lock/channel enumeration: NO true AB-BA mutex or channel cycle; best candidate = synchronous LSM stall inside try_snapshot_async holding ledger_state.write()
- [#767 Slow-demotion cascade mechanics (2026-06-16)](issue-767-slow-demotion-cascade.md) — peer_failed(Slow) demotes Hot→Cold WITHOUT tearing down the still-alive TCP mux (cleanup_dead_connections won't reap it) → inconsistent peer state + mass reconnect storm on apply pause
- [#767 live-apply permanent wedge (2026-06-16)](issue-767-live-apply-wedge.md) — NO static cycle found; compound runtime: save_utxo_snapshot() without block_in_place pins tokio worker >5s → fetched_blocks fills → spsDeactivateTimeout fires → all BlockFetch peers demote → no fetchers → permanent. Fix: block_in_place at epoch.rs:504
- [#767 residual stall Lens C (2026-06-16)](issue-767-residual-stall-lens-c.md) — apply doesn't consume because ValidateAll Alonzo fills fetched_blocks_rx (cap=1024) → blockfetch workers park on send().await → 60s FETCH_RANGE_TIMEOUT fires → mass Slow failure → governor demotes all hot peers → no fetchers; self-recovers when re-promoted. Cascade is APPLY-LAG-TRIGGERED: fix = distinguish channel-full backpressure from peer-slow timeouts
- [#767 residual fix adversarial review (2026-06-16)](issue-767-residual-stall-proposed-fix-review.md) — Fix 1 (hoist chain_db.read outside candidate_chains.write in sync.rs:5371) CONFIRMED real and correct. Fix 2 WILL NOT COMPILE: governor.rs:213 record_demote is PRIVATE fn; needs pub wrapper before it can be called from mod.rs. Fix 2 also misdescribes peer going Warm (it goes Cold via peer_failed→demote_to_cold). Fix 3 targets hypothetical send-park path; real Slow events are from ChainSync bearer errors (48 simultaneous), not blockfetch FETCH_RANGE_TIMEOUT on concurrent peers (only ONE active fetcher). Fix 4 safe as defense-in-depth.
