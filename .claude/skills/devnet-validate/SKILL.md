...
4b. **Script-context correctness** — `17-context-inspecting` (#969). Every OTHER Plutus category drives a validator that returns a constant, which proves dugite builds a context, resolves the redeemer pointer and charges a budget, but never that the context's *contents* are right — a wrong field, order or missing entry passes all of them, and #772 was exactly that. These six drive IntersectMBO's own plutus-tx validators, which read the context and fail if it does not describe the transaction being validated. Note especially:
   - `17d`/`17e` are the same script with one byte of redeemer changed and opposite verdicts, so "the script accepted it" cannot be confused with "the script never really ran".
   - `17f` brackets dugite's phase-2 CPU accounting against cardano-api's evaluator from BOTH sides — accepted at exactly the reference ExUnits, rejected one step below. That is the only assertion in the gate about evaluation *cost* rather than verdict, and #772 was a cost bug that changes no verdict under generous limits.
