...
Captured on **2026-08-12** from commit [`59368e9`](https://github.com/MichaelJFazio/dugite/commit/59368e9) on `main` (GitHub Actions `ubuntu-latest`).
...
> The collapsed _Raw measurements_ blocks contain the filtered `cargo bench` output (cargo build chatter and ANSI escapes are stripped). Full unfiltered logs are uploaded as the `benchmark-results-160` workflow artifact.
