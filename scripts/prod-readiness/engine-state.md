...
## In-progress
- item: #26b gov-action V3-ScriptContext MAP ORDERING ROOT-CAUSED (diagnose w7bg9vcwg, conf 0.93, raw-source cardano-ledger ebed62de + plutus 1.65.0.0). All 3 V3 gov-action fields = LEDGER order Script<Key in Haskell (Conway transGovAction: transMap=unsafeFromList∘map∘Map.toList preserves the ledger Map Credential Ord; members_to_remove=Set.toList Script<Key+deduped; plutus AssocMap ToData no-sort) — dugite produces KEY<Script on all 3 → DIVERGES (live phase-2 ScriptContext for any V3 script inspecting a TreasuryWithdrawals/UpdateCommittee proposal). encode/governance.rs INERT (raw_body_cbor/KeepRaw preserved on honest ingest; re-encode forge/synthetic-only) → optional separate dugite-serialization follow-up, NOT consensus. NEXT: FIXING — sort TreasuryWithdrawals/members_to_add/members_to_remove by Credential::cmp_ledger (Script<Key, mirrors #26 ledger_ordered_withdrawals) in populate_gov.rs gov_action_to_data, 1 crate dugite-uplc + mixed-cred ordering tests. [#33 DONE b64cbfa15e; #24 BLOCKED-on-Haskell-oracle; remaining [L]: #29-order / #31-F/G/H / #28b-cli + encode/governance.rs optional.]
