...
## In-progress
- item: #33 WriteBits maximumInputLength FIXING DONE (bitwise_max_input_enforced()=matches!(D|E) + writeBits 4096-byte INPUT cap before the index loop; writeBits-ONLY, purely additive 1 crate; fmt+clippy+nextest+conformance 1470/1470 INDEPENDENTLY verified; 7 new tests). NEXT: GAUNTLET #33 (fix UNCOMMITTED). [#0/#1 resolved per frontier; #24 PARKED-BLOCKED-on-Haskell-oracle; common.rs = ep246 scratch loose-end.]
