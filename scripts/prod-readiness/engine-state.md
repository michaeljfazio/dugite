...
## In-progress
- item: #28b PlutusData encoder chunk-64 DONE — GAUNTLET w6cnrst7m PASSED 0/3 (GOLD-STANDARD, raw-source-confirmed plutus d91c23d); COMMITTED d05bb21b77 (1 crate dugite-serialization). encode_bounded_plutus_bytes (reuses #28 decoder const) at 2 plutus-leaf sites; generic encode_bytes untouched. INERT verdict REFINED: encode_plutus_data IS live-reachable (re-encode branches when raw absent) but honest ingest always uses raw wire → corrective-not-regression. *** #28 + #28b BOTH closed (plutus 64-byte leaf rule byte-exact both directions). NEXT: SCHEDULE the next open item — re-assess phase-2 residuals #15 [M->H] / #24 [M] (run phase2_repro at HEAD FIRST per the #481 stale-dump lesson) vs the [L] cluster (#29-order, #26b, #31-F/G/H, #28b-cli). [#0/#1 resolved per frontier; common.rs = ep246 scratch loose-end.]
