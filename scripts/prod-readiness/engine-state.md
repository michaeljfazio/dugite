...
## In-progress
- item: #24 SPEND over-cost ANALYZED at HEAD (conf 0.95 term-level / 0.55 exact-node; analyze wvdwf3vt0, aiken 2nd-oracle). *** TERM-LEVEL not CEK/cost-model: aiken (independent CEK) reproduces dugite mem EXACTLY (1,088,456) on dugite's applied term, OVER on-chain declared (1,073,888); Spend@2/Mint@0 in the SAME tx byte-exact == declared → dugite CEK+cost-model CORRECT, over-cost is in the TERM. *** ScriptContext Data RULED OUT: node-by-node diff vs a Python transTxInfoV2 reconstruction = byte-IDENTICAL across ALL 12 V2 TxInfo fields (inputs/outputs/refs sorting, spent-output Address/Value/datum, redeemers-map) — the diagnose input-ordering/Address/Value/redeemers suspects ALL eliminated. *** NARROWED TO: dugite FLAT SCRIPT DECODER (flat/decode.rs decode_term / program.rs from_flat) yielding a term that reduces in ~145 MORE CEK steps than cardano-node's decode of the same ref-script flat bytes (LAST unexcluded component). NEXT: DECISIVE experiment — `aiken uplc decode --cbor` the ORIGINAL ref-script flat bytes, diff the term vs dugite decode + compare aiken-on-original-flat budget to declared; if aiken-original==declared < dugite → flat decoder is the bug. Falsify alt first: dump cost_models_cbor version mismatch via Koios pparams. [#0/#1 resolved per frontier; common.rs = ep246 scratch loose-end.]
