...
## In-progress
- item: #33 WriteBits maximumInputLength ROOT-CAUSED (conf 0.9; diagnose w3r7v380b, raw-source-pinned plutus 0c3d72dc). Plutus writeBits caps the INPUT bytestring at maximumInputLength=4096 bytes (Bitwise.hs:81), gated by `ensurable = matches!(D|E)` (Builtins.hs:2725) — A/B/C no cap, D/E → `BS.length b > 4096 → builtinResultFailure` (BEFORE the per-index check). ONLY writeBits carries the cap (the other bitwise builtins' D/E branch is a CByteString COSTING change, NOT a result cap — do NOT cap them; replicateByte/integerToByteString already cap 8192 correctly). The SemanticsVariant is already threaded into denote (#32). FIX: add semantics.rs bitwise_max_input_enforced()=matches!(D|E) + a writeBits-arm guard (variant.bitwise_max_input_enforced() && bs.len()>4096 → builtin_failure) BEFORE the index loop. NEXT: FIXING #33. [#0/#1 resolved per frontier; #24 PARKED-BLOCKED-on-Haskell-oracle; common.rs = ep246 scratch loose-end.]
