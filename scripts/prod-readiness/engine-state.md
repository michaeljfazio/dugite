...
## In-progress
- item: #24 SPEND over-cost BLOCKED (conf 0.8; diagnose w0jsp1jbm) — needs a HASKELL uplc CEK step-trace to localize, NOT available (ghc/cabal/nix absent; aiken is a 2nd Rust UPLC == dugite; cardano-cli can't trace an isolated term). EXHAUSTIVELY characterized over 5 wakes: dugite is BYTE-EXACT with the aiken Rust reference at EVERY layer — flat decoder, ScriptContext Data (node-by-node), cost-model coefficients (Koios ep81 0/175), ExMemoryUsage size rules (instance-by-instance vs ExMemoryUsage.hs), AND CEK step-charging rules (vs Cek/Internal.hs: same 9 StepKinds, Error/Return/forced-builtin free). The +14,568 mem = +145 CEK steps (10,502 vs 10,357) + 68-mem extra builtin CALLS = the term REDUCES 145 MORE times under the Rust family (dugite==aiken) than under Haskell — a reduction-TRACE difference no charging-rule source-read can localize. REAL consensus divergence (Babbage UTXOS runs at the DECLARED budget → dugite would fail an is_valid=true tx). UNBLOCK: build IntersectMBO/plutus uplc for ep81 PV2, `uplc evaluate --counting` on dugite's exact applied term, diff per-StepKind/per-builtin counts vs the dugite tally. PARKED. NEXT: SCHEDULE #33 [L] (WriteBits D/E maximumInputLength — SemanticsVariant already threaded from #32) → DIAGNOSE; then #29-order/#26b/#31-F/G/H/#28b-cli. [#0/#1 resolved per frontier; common.rs = ep246 scratch loose-end.]
