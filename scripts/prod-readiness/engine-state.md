...
## In-progress
- item: #31-D dup-MAP-key reject ROOT-CAUSED (conf 0.83; diagnose wllv1b1zv, raw-source-pinned cd8b7fab). NEXT: FIXING #31-D (for_each_field_entry dup-key reject for the 3 un-gated SparseKeyed sites). [NOTE: standing-prompt #1 (ep57) + #0 (ep246) are BOTH RESOLVED per the ledger.preprod/mainnet frontiers — ep57 per-cred byte-exact vs Koios (apply_utxo_changes RULED OUT), ep246 reserves/treasury diff-0 post-MIR-fix 8c868271c9. common.rs +218 = uncommitted ep246 regression-guard test (loose end, NOT an active item).]
