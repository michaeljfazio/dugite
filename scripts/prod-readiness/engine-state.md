...
## In-progress
- item: PHASE-2 RE-ASSESSMENT at HEAD DONE (wake472, phase2_repro over 769 #730 dumps, 5s): 502 MATCH / 267 DIVERGE. *** #15 (306 Error-term) RESOLVED — ZERO Error-term divergences at HEAD (#481 lesson vindicated; wake322 serialiseData resolution holds). *** #24 (inline-datum over-cost) SHRUNK 184→~50 budget-exhausted. *** NEW #32 [H] FILED: consByteString lenient-vs-strict BuiltinSemanticsVariant — 215 PV8/Babbage dumps where dugite OVER-REJECTS ("cons byte must be 0..=255") valid on-chain scripts = a LIVE Babbage-era phase-2 consensus divergence (the single biggest class). NEXT: SCHEDULE #32 → DIAGNOSE (the Plutus BuiltinSemanticsVariant PV-gate for consByteString mod-256-vs-range-check). [#0/#1 resolved per frontier; common.rs = ep246 scratch loose-end.]
