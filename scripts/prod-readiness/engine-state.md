...
## In-progress
- item: #31-C Conway PV9+ set-dedup GAUNTLET REFUTED 2/3 (DECISIVE over-rejection — 5 of 18 sites are NOT PV9+ no-dup: witness vkey(0)/native(1)/bootstrap(2)/plutus_data(4) reuse Alonzo TxWits, gated @PV12 NOT @9 [live mainnet=PV11 → would HALT sync], + block invalid_transactions is a plain LIST not a set). → FIXING attempts:2 (revert those 5 to lenient read_set; KEEP the other 13 strict). Fix stays in-tree as the rework base, UNCOMMITTED.
