...
## In-progress
- item: #28b PlutusData encoder chunk-64 ROOT-CAUSED (conf 0.9; diagnose w4ysmoe5o, raw-source-pinned plutus d91c23d). VERDICT: INERT (no live consensus divergence — all 4 hash families use raw wire / the already-chunking UPLC encoder) but a REAL round-trip self-inconsistency (encode >64B PlutusData leaf → dugite's own #28 decoder FAILS). NEXT: FIXING #28b (encode_bounded_plutus_bytes at 2 sites: cbor.rs:296 Bytes + :139 bignum magnitude; generic encode_bytes untouched). [#0/#1 resolved per frontier; common.rs = ep246 scratch loose-end.]
