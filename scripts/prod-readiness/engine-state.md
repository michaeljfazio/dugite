...
## In-progress
- item: #24 PlutusV2 SPEND-ScriptContext over-cost RE-DIAGNOSED at HEAD (conf 0.86; diagnose w94vziez3) — wake326 root-cause REFUTED. NOT a fixed +4230 delta, NOT every-inline-spend, NOT a dump artifact. CONFIRMED REAL (769/769 dumps UTxO-COMPLETE; 51 budget-exhaust, all is_valid=true on-chain → WOULD break mainnet Babbage on popular DEX spends). Re-scoped: SPEND-purpose ScriptContext over-cost in DISCRETE FIXED-PER-OUTPUT-SHAPE clusters (22x +14568mem, 16x +5526, 13x +69303…), VALIDATOR- and DATUM-SIZE-INDEPENDENT, tied to the spent output shape (addrtype=3 base-addr[script+stake] + multiasset[2] + inline datum); 69 Spend + 1 Mint divergent / 71 byte-exact in same txs / 1 under-costs. Translators + cost model Haskell-faithful → bug is in the SPEND ScriptContext build (Address-stake-cred OR multiasset-Value-Data OR TxInInfo-ordering via findOwnInput). NEXT: ANALYZE — second-oracle (cardano-node/aiken-uplc) per-builtin trace on ONE cluster dump (126af7b3 Spend@3) to byte-pin the exact differing Data node, OR inspect dugite Value/Address translators vs Haskell transTxOutV2/transAddr. [#0/#1 resolved per frontier; common.rs = ep246 scratch loose-end.]
