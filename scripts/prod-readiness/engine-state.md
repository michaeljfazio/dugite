...
## In-progress
- item: #24 SPEND over-cost — cause CLASS-PINNED to builtin-result-mem-SIZE accounting (conf 0.85; pin wb86tiwa7). *** DECISIVE: dugite is BYTE-EXACT with the aiken Rust-UPLC reference at EVERY layer — flat decoder (aiken decode == dugite decode, byte-identical), ScriptContext (node-by-node ==), cost model (Koios ep81 epoch_params 0/175 differ vs dump), CEK (3 Rust CEKs all give mem 1,088,456). On-chain Haskell = 1,073,888. So the +14,568 mem is a RUST-CEK-FAMILY (dugite+aiken) over-count vs Haskell plutus = a builtin-result-mem-SIZE accounting difference (+14568 = 145 steps×100 + a 68-word residual on ONE builtin among headList/tailList/sndPair/unConstrData). RULED OUT: decoder / cost-model / ScriptContext / dump-artifact / dugite-specific (aiken shares it). NEXT: focused RAW-source DIAGNOSE comparing dugite builtin/sized.rs (value→mem-size) vs Haskell plutus ExMemoryUsage (BuiltinList cons-cell / BuiltinPair / Data node / Integer word-size) to find the +68-word divergent type (NO Haskell runtime needed) → surgical fix gated so the 71 byte-exact controls + 999 conformance goldens stay green. [#0/#1 resolved per frontier; common.rs = ep246 scratch loose-end.]
