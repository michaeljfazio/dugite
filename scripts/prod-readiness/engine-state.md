...
## In-progress
- item: #31-C Conway PV9+ set-dedup FIXING REWORK DONE (attempts:2 — reverted the 5 over-strict sites to lenient read_set [witness vkey/native/bootstrap/plutus_data are @PV12-gated via Alonzo TxWits reuse; block invalid_transactions is a plain list — ALL raw-source-reconfirmed @ cd8b7fab]; KEPT 13 strict; fmt+clippy+nextest 1198/1198 INDEPENDENTLY re-verified; diff=1 crate). NEXT: re-GAUNTLET #31-C on the corrected 13-strict/5-lenient split (incl. a retain-both-vs-dedup consensus-equivalence lens). Fix UNCOMMITTED.
