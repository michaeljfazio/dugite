...
## In-progress
- item: #26b gov-action V3-ScriptContext MAP ORDERING FIXING DONE — all 3 V3 gov-action fields now ledger-ordered Script<Key in populate_gov.rs gov_action_to_data (TreasuryWithdrawals reuses the #26-gauntlet-proven ledger_ordered_withdrawals helper; members_to_add + members_to_remove sort_by Credential::cmp_ledger; members_to_remove deduped for Set parity). 1 crate dugite-uplc, populate_gov.rs ONLY (+292/-26); credential Data tags unchanged (only entry order). 6 new tests (3 mixed-cred Script-first + 3 single-entry identity). INDEPENDENTLY re-verified fmt + clippy -p dugite-uplc -D warnings + nextest -p dugite-uplc = 469/469. NEXT: GAUNTLET #26b (fix UNCOMMITTED). [#33 DONE b64cbfa15e; #24 BLOCKED; remaining [L]: #29-order / #31-F/G/H / #28b-cli + encode/governance.rs optional.]
