...
26b. [M][phase2/governance][ROOT-CAUSED wake516, from #26 excluded-scope] V3 gov-action ScriptContext MAP ORDERING. populate_gov.rs gov_action_to_data builds 3 V3 GovernanceAction fields in dugite KEY<SCRIPT order — TreasuryWithdrawals `Map V2.Credential Lovelace` (Constr 2 fld 0, ~:263 iterates withdrawals:BTreeMap<reward-blob 0xE_key<0xF_script>), UpdateCommittee members_to_add `Map ColdCred Epoch` (Constr 4 fld 2, ~:305 iterates BTreeMap<Credential> derived Key<Script), members_to_remove `[ColdCred]` (Constr 4 fld 1, ~:299 iterates Vec<Credential> wire order) — but Haskell V3 = LEDGER order SCRIPT<KEY on all 3 (Conway TxInfo.hs transGovAction 666-676: transMap=unsafeFromList∘map∘Map.toList preserves the ledger Map Credential Ord ScriptHashObj<KeyHashObj; members_to_remove=Set.toList Script<Key+deduped; plutus 1.65.0.0 AssocMap ToData no-sort; raw-source cardano-ledger ebed62de). DIVERGES on all 3 → live phase-2 ScriptContext byte-divergence for any V3 script inspecting a TreasuryWithdrawals/UpdateCommittee proposal. SAME transMap mechanism as the #26-shipped V3 txInfoWdrl (ledger_ordered_withdrawals, Script<Key); V3-ONLY so ledger order throughout (NOT the V1/V2 Plutus Key<Script — gov actions never exist in V1/V2; the #26-v1 trap does not apply). encode/governance.rs gov-action CBOR re-encode (:152 withdrawals / :178 members_to_remove / :182 members_to_add / :41 voting) ALSO mis-orders but INERT (honest ingest preserves raw_body_cbor via KeepRaw; block.rs:121-124/191-194 emit raw verbatim; re-encode only the encode_transaction_body_for_era ELSE-branch for synthetic txs; tx/block-body hash over raw wire) → optional separate dugite-serialization follow-up, NOT consensus. FIX (Tier A', dugite-uplc populate_gov.rs ONLY): sort all 3 fields by Credential::cmp_ledger (Script<Key + 28B hash asc — the #26 comparator) before building Data::Map/Data::List; credential Data tags already correct (only order changes); dedup members_to_remove for Set parity (no-op for valid txs). how_to_confirm: mixed key+script TreasuryWithdrawals/members_to_add/members_to_remove → Data Script-first; single-entry = identity. 0/769 corpus gov-action dumps → gate = Haskell-Ord-by-construction match + ordering tests (the #26/#29/#30 standard; transMap=Map.toList + AssocMap no-sort + ledger Credential Ord chain dispositive without a dump). state:DONE wake524 (COMMITTED 8685e95e56; gauntlet wab7h0rjj PASSED 0/3 GOLD-STANDARD raw-source cardano-ledger ebed62de + plutus 1.65.0.0 — all 3 V3 gov-action fields ledger Script<Key via transMap=Map.toList+Set.toList, AssocMap no-sort, V3-only #26-v1-trap-inapplicable, encode/governance.rs INERT, dedup safe; 1 crate dugite-uplc populate_gov.rs ONLY; closes the entire #26-excluded gov-map-ordering scope) [was FIXING wake520 (the 3 sorts applied in populate_gov.rs gov_action_to_data — TreasuryWithdrawals reuses ledger_ordered_withdrawals [#26-proven]; members_to_add + members_to_remove sort_by cmp_ledger; remove deduped; 1 crate dugite-uplc +292/-26; credential tags unchanged; 6 new tests; INDEPENDENTLY re-verified fmt+clippy -D warnings+nextest 469/469; UNCOMMITTED, gauntlet next)] [was ROOT-CAUSED wake516 conf:0.93] (diagnose w7bg9vcwg raw-source cardano-ledger ebed62de + plutus 1.65.0.0: all 3 V3 gov-action fields = ledger Script<Key via transMap=Map.toList + Set.toList; dugite Key<Script → DIVERGES; encode/governance.rs INERT; fix = sort 3 fields by cmp_ledger in populate_gov.rs).
32. [H][phase2/uplc][NEW wake472, phase2_repro-confirmed at HEAD] consByteString BuiltinSemanticsVariant — dugite OVER-REJECTS valid Babbage (PV8) scripts. 215/769 #730 dumps diverge "consByteString: cons byte must be 0..=255" (ALL protocol_major=8, is_valid=true on-chain). dugite crates/dugite-uplc/src/builtin/denotations.rs:144-159 HARDCODES the V2+ strict range-check (bigint_to_u8) regardless of BuiltinSemanticsVariant — comment "We follow V2 semantics by default (which is mainnet)". Haskell consByteString = (i mod 256) BS.cons bs (lenient/mod-256) under the OLD BuiltinSemanticsVariant and strict range-check under the newer variant; the variant is selected by language(V1/V2/V3)+protocol-version, and at Babbage PV8 the relevant scripts use the LENIENT variant. dugite ignoring the variant → over-rejects on-chain-valid out-of-range cons bytes = LIVE Babbage-era CONSENSUS divergence (a from-genesis/Babbage replay fails these valid scripts → ValidationTagMismatch). FIX (Tier A', dugite-uplc): thread the BuiltinSemanticsVariant (derive from language + protocol_major — already params to eval_phase_two_raw) into the consByteString denotation; apply mod-256 for the lenient variant, range-check for the strict. SURVEY the other BuiltinSemanticsVariantA→B→C-sensitive builtins. how_to_confirm: phase2_repro over the 215 PV8 dumps → 0 diverge post-fix. state:DONE wake484 (COMMITTED 257658b360; gauntlet wsvwvp16v PASSED 0/3; phase2_repro 215→0) [was FIXING+VERIFYING wake480] (215→0 consByteString divergences in phase2_repro, 502→717 MATCH no-regression, conformance 999/999; fix uncommitted, gauntlet next) [was ROOT-CAUSED wake476 conf:0.95] (HEAD-phase2_repro-confirmed: 215/769, all PV8 is_valid=true; diagnose wntjw0b2i raw-source-pinned plutus d3c8d752/bddbf4b1 — strict ⟺ language==PlutusV3, V1/V2 lenient mod-256 at EVERY PV; fix = thread SemanticsVariant into denote, consByteString lenient rem_euclid-256 for V1/V2 [A/B/D] vs strict for V3 [C/E]; ONLY consByteString needs the denotation change).
33. [L][phase2/uplc][NEW wake484, from #32 gauntlet wsvwvp16v] WriteBits BuiltinSemanticsVariant maximumInputLength check. Plutus writeBits gains a D/E-only `maximumInputLength → builtinResultFailure` guard (variants A/B/C lack it); dugite's writeBits denotation neither implements the length check nor variant-gates it. PRE-EXISTING + ORTHOGONAL to #32 (consByteString); NOT implicated in any of the 215 Babbage-PV8/V1-V2 divergences (the bitwise builtins writeBits/readBit/etc. are unavailable at Babbage — they arrived with later Plutus / V3). Latent: would only matter for a V3 (C/E) script calling writeBits with an over-long input. With the #32 SemanticsVariant now threaded into denote, the fix is small (gate the maximumInputLength→fail in the writeBits denotation for D/E). how_to_confirm: a V3 writeBits over-length input → builtinResultFailure under D/E. Lowest priority (no observed divergence). state:DONE wake512 (COMMITTED b64cbfa15e; gauntlet wzwkl05yw PASSED 0/3 GOLD-STANDARD raw-source plutus 0c3d72dc — 4096-constant / ensurable=matches!(D|E) / strict-`>` / guard-before-index byte-exact; writeBits-ONLY purely-additive 1 crate dugite-uplc; conformance 1470/1470; LATENT — zero live impact today, defense-in-depth V3@PV11+ [E] + V1/V2@PV11+ [D]) [was FIXING wake508 (bitwise_max_input_enforced()=matches!(D|E) + writeBits 4096-byte input cap before the index loop; writeBits-ONLY purely-additive; 7 tests; fmt+clippy+nextest+conformance 1470/1470 INDEPENDENTLY verified; uncommitted, gauntlet next)] [was ROOT-CAUSED wake504 conf:0.9] (diagnose w3r7v380b raw-source-pinned plutus 0c3d72dc: maximumInputLength=4096-byte INPUT cap on writeBits ONLY, gated `ensurable`=matches!(D|E); fix = semantics.rs bitwise_max_input_enforced() + writeBits-arm guard before the index loop; variant already threaded from #32).
...
24. [M][phase2][ROOT-CAUSED wake326 (wogj8wp6h, conf 0.83)] PlutusV2 inline-datum-SPEND ExUnit over-cost (184→~50 @wake472 budget-
   ...
   mem=329275/steps=118172478. NEXT: pin the exact over-cost line (full UTxO context + CEK-work compare). *** RE-DIAGNOSED wake488 (w94vziez3, conf 0.86): wake326 root-cause REFUTED — REAL (769/769 dumps UTxO-COMPLETE, NOT an artifact) but NOT a fixed +4230 delta + NOT every-inline-spend; re-scoped to a SPEND-purpose ScriptContext over-cost in DISCRETE FIXED-PER-OUTPUT-SHAPE clusters (22x+14568mem/16x+5526/13x+69303), validator+datum-size-INDEPENDENT, tied to the base-addr(script+stake)+multiasset[2] spent-output shape; 69 Spend+1 Mint divergent / 71 byte-exact / 1 under-cost. Translators+cost-model Haskell-faithful → bug in the SPEND ScriptContext build (Address-stake-cred / multiasset-Value-Data / TxInInfo-ordering via findOwnInput). NEXT: ANALYZE — 2nd-oracle (cardano-node/aiken-uplc) per-builtin trace to pin the exact Data node. state:BLOCKED wake500 (needs a Haskell uplc CEK step-trace to localize the +145-step/+68-mem Rust-family-vs-Haskell reduction-trace divergence; ghc/cabal/nix absent → build IntersectMBO/plutus uplc out-of-band to unblock; dugite byte-exact with aiken at decoder/ScriptContext/cost-model/ExMemoryUsage/step-charging) [was ANALYZING wake496] (CAUSE CLASS-PINNED: builtin-result-mem-SIZE over-count shared by the Rust UPLC family [dugite==aiken byte-exact at decoder/ScriptContext/cost-model/CEK] vs Haskell plutus; +68-word residual on one list/pair/Data builtin; NEXT = sized.rs vs Haskell ExMemoryUsage RAW-source diff) [was ANALYZING wake492] (aiken 2nd-oracle: TERM-level not CEK [aiken==dugite mem 1,088,456 > declared 1,073,888]; ScriptContext Data RULED OUT [node-by-node == Python transTxInfoV2, all 12 fields]; NARROWED to the FLAT SCRIPT DECODER flat/decode.rs decode_term / from_flat — ~145 extra CEK steps; NEXT = aiken-decode-original-flat-vs-dugite decisive experiment) [was ROOT-CAUSED(re-scoped) wake488 conf:0.86] conf 0.95-term/0.55-node
...
## In-progress
- item: #26b gov-action V3-ScriptContext MAP ORDERING DONE — GAUNTLET wab7h0rjj PASSED 0/3 GOLD-STANDARD (all 3 lenses raw-source-reconfirmed cardano-ledger ebed62de + plutus 1.65.0.0: all 3 V3 gov-action fields = ledger Script<Key via transMap=unsafeFromList∘map∘Map.toList + Set.toList, AccountAddress-Ord collapses to Credential-Ord [per-tx network dropped], plutus AssocMap ToData no-sort, Credential derived Ord Script<Key; V3-only ⇒ #26-v1 Plutus-Key<Script trap cannot apply; encode/governance.rs confirmed INERT [tx-id=blake2b256(raw_body_cbor) KeepRaw]; dedup safe [read_set_strict rejects byte-dups]; purely-ordering, honest txs unaffected). COMMITTED 8685e95e56 (1 crate dugite-uplc, populate_gov.rs ONLY; common.rs ep246 scratch EXCLUDED). NEXT: SCHEDULE the next open item (all [L]: #29-order / #31-F/G/H / #28b-cli + encode/governance.rs optional-inert; #24 BLOCKED-on-Haskell-oracle). [#33 DONE b64cbfa15e; #0/#1 resolved per frontier; common.rs = ep246 scratch loose-end.]
  *** wake524 (ultracode): ran the #26b gauntlet (wab7h0rjj, 3 lenses, in-turn ~3.6min/209K tok) → PASSED 0/3 GOLD-STANDARD, every lens INDEPENDENTLY raw-source-reconfirmed (cardano-ledger ebed62de + plutus 1.65.0.0, raw.githubusercontent NOT WebFetch). (1) per-field ledger-order: all 3 V3 GovernanceAction fields built in ledger Script<Key — TreasuryWithdrawals `Map AccountAddress Coin` via transMap=PV3.unsafeFromList∘map∘Map.toList (TxInfo.hs L666-694; AccountAddress Ord = (network, Credential), network per-tx-constant so collapses to Credential Ord; transAccountAddress DROPS network → bare Credential, dugite's ledger_ordered_withdrawals drops network too = sound for single-network tx); members_to_add `Map ColdCred Epoch` via same transMap (L675); members_to_remove `Set ColdCred` via Set.toList (L674, Script<Key+deduped). Ledger Credential derived Ord = Script<Key (ScriptHashObj first ctor, Credential.hs L96-99) == dugite cmp_ledger (Script=0<Key=1 then hash asc). NO Plutus re-sort (AssocMap unsafeFromList=bare Map wrapper L224-226; Map ToData mapToBuiltin straight cons-fold, no sort/dedup L101-110). V3-ONLY (GovernanceAction only in PlutusLedgerApi.V3 Contexts L406-428; ctor tags TreasuryWithdrawals=2/UpdateCommittee=4 ✓) so the #26-v1 V1/V2 fresh-Plutus-Map Key<Script trap CANNOT apply → ledger Script<Key unambiguously correct for all 3; credential Data tags unchanged (transCred KeyHashObj→PubKey Constr0 / ScriptHashObj→Script Constr1 == credential_to_plutus). (2) over-reach/scope: confined to gov_action_to_data; credential tags / derived Ord / voting_procedures (already #26-sorted) / V1/V2 withdrawals_to_plutus (Plutus Key<Script, deliberate) / the other 5 gov-action arms all UNTOUCHED; encode/governance.rs INERT (honest ingest tx hash=blake2b256(raw_body_cbor) KeepRaw; GovActionId derived from that hash, encode_gov_action forge/outbound-only — deferral correct, fix NOT incomplete); dedup SAFE (members_to_remove decoded via read_set_strict which HARD-REJECTS byte-dup elements → dedup a guaranteed no-op for valid txs; Haskell Set.toList dedups too; dedup-after-sort_by collapses ALL equals); no over-sort (0/1-elem identity tests; hash tiebreak deterministic). (3) commit-safety: PURELY ORDERING (no value/cred-bytes/Constr-tag/other-field change); honest txs unaffected except the corrected order (the point — dugite now byte-MATCHES Haskell for any tx with >=2 mixed key+script gov-action entries); LATENT (0/769 corpus gov-action dumps + V3-gov-inspecting script rare on-chain → zero live impact today; defense for V3 governance scripts); by-construction gate = canonical Haskell Ord (the #26/#29/#30 standard when no reference dump exists; TreasuryWithdrawals additionally carries the #26 gauntlet pedigree via the reused helper). ALL 3 lenses flagged: stage ONLY populate_gov.rs, EXCLUDE common.rs (ep246 scratch, +218, different crate, pure test-only) + proptest-regressions/scratch artifacts. INDEPENDENTLY re-verified green (build/clippy -D warnings/fmt/nextest 469/469 incl the 6 new #26b tests). COMMITTED 8685e95e56 (1 crate dugite-uplc, populate_gov.rs ONLY +292/-26; common.rs + artifacts EXCLUDED via explicit-filename staging). #26b DONE — the 3 V3 gov-action ScriptContext map/list orderings are byte-exact with Haskell (ledger Script<Key) + adversarially verified + the entire #26-excluded gov-map-ordering scope is now CLOSED. NEXT WAKE: SCHEDULE the next open item — the [L] cluster (#29-order within-pass gov-action ratify order, #31-F/G/H decoder leniencies, #28b-cli offline) + the optional-INERT encode/governance.rs gov-action re-encode order (dugite-serialization, forge-self-consistency only); #24 remains BLOCKED-on-Haskell-oracle.
  *** wake520 (ultracode): DRIVE #26b ROOT-CAUSED→FIXING (fix Workflow w79kt37ey, in-turn ~5min/85K tok). Applied the 3 ledger-order sorts in crates/dugite-uplc/src/populate_gov.rs gov_action_to_data: (1) TreasuryWithdrawals arm — REUSES `crate::tx_info_populate::ledger_ordered_withdrawals(withdrawals)?` (the #26-gauntlet-PROVEN V3 helper; same &BTreeMap<Vec<u8>,Lovelace> input → Vec<(PrimCred,Lovelace)> sorted by cmp_ledger Script<Key), then maps each (stake,amount)→(credential_to_plutus(&stake).to_data(), Data::I(amount)) — byte-identical ledger order, minimal new code; (2) UpdateCommittee members_to_add — collect BTreeMap<Credential,u64> into a Vec, `sort_by(|a,b| a.0.cmp_ledger(b.0))`, then build add_map; (3) members_to_remove (Vec<Credential>) — `sort_by(cmp_ledger)` + `dedup()` (Set.toList parity, no-op for valid txs) before remove_list. SCOPE GUARD INDEPENDENTLY VERIFIED (git diff --stat): ONLY populate_gov.rs changed (1 file +292/-26, 1 crate dugite-uplc); credential Data TAGS unchanged (credential_to_plutus PubKey Constr0 / Script Constr1 at every site — only ENTRY ORDER changes); derived Credential Ord (credentials.rs), voting_procedures_to_plutus (already #26-sorted), V1/V2 withdrawals_to_plutus (Plutus Key<Script, deliberate), encode/governance.rs (INERT, deferred), and the ParameterChange/HardFork/NoConfidence/NewConstitution/InfoAction arms ALL UNTOUCHED. 6 new tests: gov_action_{treasury_withdrawals,update_committee_add,update_committee_remove}_orders_script_before_key (mixed key 0xE1 + script 0xF1 → Data Script-first even though the BTreeMap iterates key-first) + 3 single-entry-identity (no over-sort regression). INDEPENDENTLY re-ran (NOT trusting the agent, #438): cargo fmt -p dugite-uplc --check = clean; cargo clippy -p dugite-uplc --all-targets -D warnings = 0; cargo nextest run -p dugite-uplc = 469/469 passed 0 failed (incl all 6 new + 2 pre-existing gov_action constr tests). Tier A' (phase-2 ScriptContext). 0/769 corpus gov-action dumps + no live V3-gov-inspecting-script reference → the byte-exact gate is Haskell-Ord-by-construction (transMap=Map.toList preserve + AssocMap no-sort + ledger Credential Ord = Script<Key) + the ordering tests (the #26/#29/#30 standard; the TreasuryWithdrawals arm additionally carries the #26 gauntlet pedigree via the reused helper). NEXT WAKE: GAUNTLET #26b (3 lenses: (1) per-field Plutus-vs-ledger order — re-confirm all 3 V3 gov-action fields are ledger Script<Key from RAW Conway TxInfo.hs transGovAction + the #26-v1-trap NON-applicability [gov actions V3-only]; (2) over-strictness / scope — credential tags unchanged, no V1/V2 or non-gov-action path touched, encode/governance.rs correctly deferred-as-INERT, dedup safe; (3) commit-safety — purely-ordering, honest txs unaffected, 1 crate, exclude common.rs) → commit on pass. Fix UNCOMMITTED.
  *** wake516 (ultracode): #33 DONE last wake → SCHEDULE #26b (highest-impact OPEN item: live V3-governance phase-2 ScriptContext ordering > the latent/safe-direction decoder items #31-F/G/H + offline #28b-cli + rare-partial-fit #29-order; #24 BLOCKED), DRIVE NEW→ROOT-CAUSED (diagnose Workflow w7bg9vcwg, in-turn ~5.2min/115K tok, conf 0.93, raw-source-pinned cardano-ledger ebed62de + plutus 1.65.0.0). #26b = the 3 V3 gov-action ScriptContext map fields DELIBERATELY EXCLUDED from the #26 credential-ord fix. PINNED (per RAW Conway TxInfo.hs transGovAction 666-676 + transMap 692-694 + plutus V3 Contexts.hs GovernanceAction + AssocMap.hs no-sort proof): ALL 3 fields = LEDGER order Script<Key — (1) TreasuryWithdrawals `Map V2.Credential Lovelace` via transMap=unsafeFromList∘map∘Map.toList over the ledger `Map AccountAddress Coin` (preserves ledger Credential Ord ScriptHashObj<KeyHashObj = the SAME mechanism as the #26-shipped V3 txInfoWdrl `ledger_ordered_withdrawals`); (2) UpdateCommittee members_to_add `Map ColdCred Epoch` via the same transMap over the ledger `Map (Credential ColdRole) EpochNo`; (3) members_to_remove `[ColdCred]` via Set.toList (Script<Key, deduped). This is V3-ONLY → ledger order throughout (the OPPOSITE of V1/V2 txInfoWdrl Plutus Key<Script — the #26-v1 REFUTATION trap, correctly NOT repeated: gov actions never exist in V1/V2). DUGITE GAP (HEAD-verified populate_gov.rs gov_action_to_data this wake): all 3 produce KEY<Script — TreasuryWithdrawals (:263) iterates withdrawals:BTreeMap<reward-blob> in raw-header order 0xE_(key)<0xF_(script); members_to_add (:305) iterates BTreeMap<Credential,epoch> in DERIVED Credential Ord (Key=0<Script=1); members_to_remove (:299) iterates Vec<Credential> in wire order (unreliable). encode/governance.rs (:152/:178/:182/:41) ALSO mis-orders but INERT (block.rs:121-124/191-194 emit raw_body_cbor verbatim; re-encode only via encode_transaction_body_for_era ELSE-branch for synthetic txs; tx/block-body hash = blake2b256(raw wire) via KeepRaw). FIX (Tier A', dugite-uplc populate_gov.rs ONLY): sort all 3 fields by Credential::cmp_ledger (Script<Key + hash asc — the #26 comparator) before building Data::Map/Data::List; credential Data TAGS already correct (PubKey Constr0 / Script Constr1) — only ENTRY ORDER changes; dedup members_to_remove for Set parity (no-op for valid txs). TESTS: mixed key+script TreasuryWithdrawals/members_to_add/members_to_remove → Data Script-first; single-entry = identity (no over-sort regression). REFERENCE: 0/769 corpus gov-action dumps (all Babbage spend/mint) + a V3-script-inspecting-gov-action tx is vanishingly rare on-chain → gate = Haskell-Ord-by-construction match + ordering tests (the standard accepted in #26/#29/#30; the transMap=Map.toList + AssocMap no-sort + ledger Credential Ord chain is dispositive without a dump). state:ROOT-CAUSED attempts:0 conf:0.93. NEXT WAKE: SCHEDULE #26b FIXING (sort 3 fields by cmp_ledger in populate_gov.rs + mixed-cred ordering tests; fmt/clippy/nextest; then GAUNTLET with the #26-style lenses incl. the V1/V2-vs-V3-order trap re-confirmation).
  *** wake512 (ultracode): ran the #33 gauntlet (wzwkl05yw, opus 3-lens refute-by-default panel, in-turn) → PASSED 0/3 GOLD-STANDARD, every lens raw-source-reconfirmed (plutus 0c3d72dc). (1) ensurable-match/constant/order: maximumInputLength = EXACTLY 4096 (Bitwise.hs:81-83; 8192 is maximumOutputLength for integerToByteString/replicateByte, NOT the input cap); gate `ensurable = D->True|E->True|_->False` (Builtins.hs:2725-2730) == dugite matches!(D|E) (A/B/C no cap); guard `BS.length b > 4096` is a STRICT `>` on the INPUT bytestring and fires BEFORE the per-index out-of-bounds check (an over-length input fails even with empty/in-range indices) — dugite placed the guard before the idx loop; builtinResultFailure == dugite builtin_failure (recoverable). (2) over-strictness/writeBits-only/conformance: the cap is added to writeBits ONLY — the other bitwise builtins (readBit/and/or/xor/complement/shift/rotate/countSetBits/findFirstSetBit) carry NO maximumInputLength result-cap in Haskell (their D/E difference is a CByteString COSTING newtype only); dugite added no length cap to any of them (diff = the writeBits arm + the new helper, nothing else); replicateByte(8192)/integerToByteString(8192) UNTOUCHED; variant C (V3@PV10) is NOT capped (matches!(D|E) excludes C) → a >4096 input returns Ok under C; conformance 1470/1470 (the cap is a no-op — all vectors <4096); normal <=4096 inputs byte-identical under all variants. (3) commit-safety: purely additive (163 insertions, 0 deletions); honest scripts with <=4096-byte writeBits inputs eval IDENTICALLY (the guard is a no-op below the threshold); dugite only rejects what Haskell ALSO rejects (correctness alignment); LATENT — the bitwise builtins are unavailable pre-PV10/V3 (absent from the Babbage preprod dumps) so ZERO live impact today; defense-in-depth for V3 scripts @PV11+ (variant E) and V1/V2 @PV11+ (variant D); all 3 lenses flagged: stage ONLY semantics.rs + denotations.rs, EXCLUDE common.rs. COMMITTED b64cbfa15e (2 dugite-uplc files, 1 crate; common.rs ep246 scratch + the cbor_proptest.proptest-regressions artifact EXCLUDED via explicit-filename staging). #33 DONE — the WriteBits D/E maximumInputLength cap is byte-exact with Haskell + adversarially verified + conformance-preserved; latent (correct for future V3/V1-V2 PV11+ scripts, zero current divergence). NEXT WAKE: SCHEDULE the next open item — the [L] cluster (#29-order, #26b, #31-F/G/H, #28b-cli); #24 remains BLOCKED-on-Haskell-oracle (needs a Haskell `uplc evaluate --counting` step-trace; ghc/cabal/nix absent).
  *** wake508 (ultracode): DRIVE #33 ROOT-CAUSED→FIXING (fix Workflow wblte27aw, in-turn ~7.5min/72K tok). Added SemanticsVariant::bitwise_max_input_enforced(self)=matches!(D|E) (semantics.rs:129, mirrors Haskell `ensurable`) + the writeBits 4096-byte INPUT cap guard (denotations.rs:937-941: `if variant.bitwise_max_input_enforced() && bs.len() > 4096 { return Err(builtin_failure(id, "writeBits: input too long (maximum is 4096 bytes)")) }`) placed AFTER unwrap bs/idx-list + BEFORE the index loop (Haskell guard order). SCOPE GUARD INDEPENDENTLY VERIFIED: writeBits-ONLY — the only production 4096/WRITE_BITS_MAX is the writeBits arm; NO cap leaked to readBit/and/or/xor/complement/shift/rotate/countSetBits/findFirstSetBit; replicateByte(8192)/integerToByteString(8192) UNTOUCHED; diff purely ADDITIVE (163 insertions, 0 deletions, 1 crate dugite-uplc: denotations +145 / semantics +18). 7 new tests (semantics helper false-A/B/C-true-D/E; writeBits over-len@D/E→fail, @C→Ok, boundary 4096-Ok/4097-fail [`>` not `>=`], normal-32B-identical-C/E, length-guard-precedes-index, empty-indices-over-len@E→fail). INDEPENDENTLY re-ran fmt(scoped)+clippy -p dugite-uplc --all-targets -D warnings+nextest -p dugite-uplc --features conformance = 1470/1470 (999 conformance vectors green; the cap is a no-op for them — all <4096). NEXT WAKE: GAUNTLET #33 (3 lenses: per-variant Haskell-ensurable match + the 4096-constant + guard-order; over-strictness / writeBits-only / conformance-preserved; commit-safety) → commit on pass. Fix UNCOMMITTED.
  *** wake504 (ultracode): #24 PARKED-BLOCKED last wake → SCHEDULE #33, DRIVE NEW→ROOT-CAUSED (diagnose Workflow w3r7v380b, in-turn ~4min/77K tok, conf 0.9, raw-source-pinned plutus 0c3d72dc). PINNED: maximumInputLength=4096 bytes (Bitwise.hs:81-83); writeBits guard `BS.length b > 4096 → builtinResultFailure` (Builtins.hs:2191-2221, the `ensurable` branch) fires on INPUT length BEFORE the per-index out-of-bounds checks. VARIANT GATE = `ensurable = \case D->True; E->True; _->False` (Builtins.hs:2725-2730) → A/B/C no cap, D/E cap = exactly matches!(D|E), confirms the #32 gauntlet. SCOPE: ONLY writeBits carries the cap (grep: maximumInputLength in 1 denotation). The other bitwise builtins (readBit/and/or/xor/complement/shift/rotate/countSetBits/findFirstSetBit) D/E branch ONLY swaps a CByteString COSTING newtype (cost-model, NOT a result cap) — must NOT add a length cap (would diverge). replicateByte (8192 count) + integerToByteString (8192 width) already cap correctly + un-gated, matching Haskell. AVAILABILITY (CORRECTS the #32 "V3-only" hint): batch5 bitwise builtins available V3@PV10→C (no cap) and V3/V1/V2@PV11+→D/E (cap) per Versions.hs builtinsIntroducedIn — so matches!(D|E) is EXACT (the only reachable no-cap case is C=V3@plomin). DUGITE GAP: writeBits (denotations.rs:922-958) does the per-index check but NO input-length cap + IGNORES the (already-threaded) `variant`. FIX (surgical, ~12 lines, writeBits-ONLY): (1) semantics.rs add bitwise_max_input_enforced(self)=matches!(D|E) (mirrors Haskell `ensurable`); (2) the WriteBits arm, after unwrap bs + BEFORE the index loop: `if variant.bitwise_max_input_enforced() && bs.len() > 4096 { return Err(builtin_failure(id, "writeBits: input too long (maximum is 4096 bytes)")) }`. TESTS: over-len@E/D→fail, @C→Ok, boundary 4096 Ok/4097 fail, normal-len identical both, over-len+in-range-idx→still length-fail (guard order), semantics-helper test; conformance 999 stay green (default=E, vectors <4096). [L]/latent (bitwise builtins absent from the Babbage preprod dumps). NEXT WAKE: SCHEDULE #33 FIXING.
  *** wake500 (ultracode): DRIVE #24 the focused mem/step source-comparison DIAGNOSE (Workflow w0jsp1jbm, in-turn ~10min/172K tok, conf 0.8; instrumentation REVERTED, tree clean) → verdict BLOCKED. EXACT reconciliation: Spend@3 dugite mem=1,088,456 (10,502 steps×100 + builtin 38,156 + startup 100) vs declared 1,073,888; the +14,568 = +145 CEK steps (10,502 vs 10,357) + 68-mem from EXTRA builtin CALLS (NOT higher per-call mem). Control Spend@2 (same TxInfo) byte-exact 831,789==declared. *** RULED OUT BY RAW-SOURCE READING (all 3 candidate rule-classes byte-identical to Haskell): (1) STEP-CHARGING — dugite compute_step (machine/cost.rs:220 cost_for:97) == Haskell stepAndMaybeSpend (Cek/Internal.hs:833-874): same 9 StepKinds, Error charges 0, Return/forceEvaluate/applyEvaluate charge 0, forced-builtin = Force+Builtin+Apply 3 steps in BOTH. (2) ExMemoryUsage — dugite size_of_* (cost.rs:43-260) == ExMemoryUsage.hs instance-by-instance (Integer log2/64+1, ByteString (len-1)quot8+1, List=len, Data 4/node both-k-and-v, DataNodeCount 1/node, BLS, all wrappers); only INERT Pair=maxBound-vs-1 differs (never feeds a model — all pair builtins Constant mem). (3) MEM-MODEL SHAPE — heavy builtins (headList/tailList/sndPair/fstPair/unConstrData/chooseList all Constant mem=32; equals*/ifThenElse Constant=1) size-INDEPENDENT in BOTH; no shape/sized-arg mismatch. *** So the +145 steps come from the TERM REDUCING 145 MORE times under the Rust family (dugite==aiken, proven byte-exact at every layer) than under Haskell — a CEK reduction-TRACE difference (NOT a charging rule) that source-reading the charging code CANNOT localize. NEEDS a Haskell `uplc evaluate --counting` per-StepKind/per-builtin trace on dugite's exact applied term — UNAVAILABLE (ghc/cabal/nix all `not found`; aiken is a 2nd Rust UPLC already == dugite; cardano-cli can't trace an isolated term). REAL consensus divergence (Babbage UTXOS evaluateScriptRestricting at the DECLARED budget → dugite marks an is_valid=true tx phase-2 FAILED). DECISION: PARK #24 as BLOCKED-on-Haskell-oracle; the 5-wake chain (gather→diagnose→analyze→pin→mem-diagnose) is EXHAUSTIVE — dugite is byte-exact with the entire Rust UPLC ecosystem (aiken); the residual is a Rust-vs-Haskell reduction-trace subtlety the aiken oracle CANNOT catch (it SHARES the trace). UNBLOCK PATH (out-of-band, needs ghc 9.6.x+cabal like the conformance-corpus build): build IntersectMBO/plutus uplc → `uplc evaluate --counting` on the captured Spend@3 applied term → diff per-StepKind counts vs the dugite tally captured this wake → the 145-step delta localizes. NEXT WAKE: SCHEDULE #33 [L] (WriteBits D/E maximumInputLength→builtinResultFailure — the SemanticsVariant from #32 is already threaded, so it is a small clean fix) → DIAGNOSE. LESSON BANKED: a NODE must match Haskell's CEK reduction TRACE exactly, not just the charging rules + the aiken reference — and pinning a reduction-trace divergence REQUIRES a Haskell CEK oracle; the Rust 2nd-oracle (aiken) is NECESSARY-BUT-INSUFFICIENT (it confirms dugite==Rust-reference but cannot reveal a SHARED Rust-vs-Haskell trace bug).
  *** wake496 (ultracode): DRIVE #24 the DECISIVE pin experiment (analyze Workflow wb86tiwa7, in-turn ~27min/221K tok, aiken 2nd-oracle; tree clean). *** FLAT DECODER RULED OUT: aiken `uplc decode --cbor` of the ORIGINAL 9952-byte ref-script flat bytes == dugite's decode (Program::from_flat→to_cbor→aiken-decode) byte-for-byte IDENTICAL (diff exit 0). dugite's flat decoder produces the EXACT cardano-node/aiken term. (Cosmetic: dugite to_flat appends 1 extra 0x01 filler byte — no cost impact; minor BitWriter::write_filler quirk.) *** DECISIVE BUDGET DISAMBIGUATION: aiken-eval of AIKEN'S OWN decode of the original bytes (applied to the identical ctx) == dugite (mem 1,088,456) ≠ declared (1,073,888). THREE independent Rust CEKs agree at 1,088,456: dugite-CEK+on-chain-V2-model; aiken-CEK+default-model; aiken-CEK+the-dump's-on-chain-ep81-V2-array (eval_as). On-chain Haskell-accepted = 1,073,888. *** COST MODEL CONFIRMED CORRECT (not artifact): koios.sh preprod tx_info → block 1149332 → epoch 81; epoch_params(81) PlutusV2 175-param array BYTE-IDENTICAL to the dump cost_models_cbor (0/175 differ); ep81=PV8=VariantA. + the 2 same-tx controls (Spend@2 831789, Mint@0 1256320) byte-exact == declared. *** PINNED CAUSE = (C) a builtin-result-mem-SIZE / CEK-step over-count SHARED BY THE RUST UPLC FAMILY (dugite AND aiken/pragma uplc v1.1.21) vs the Haskell IntersectMBO/plutus evaluator on this Spend@3 DEX script shape. NOT a decoder bug, NOT a cost-model bug, NOT a dump artifact, NOT dugite-specific (aiken shares it). +14,568 mem = 145 steps×100 + a 68-word RESIDUAL ⇒ ≥1 builtin (list/pair/Data traversal: headList/tailList/sndPair/unConstrData) returns a value whose mem-SIZE the Rust CEKs compute 68 words larger than Haskell, cascading ~145 extra steps. conf 0.85 (rule-outs ~0.97; the 0.85 = could NOT run the Haskell plutus evaluator directly — but the on-chain declared 1,073,888 IS the Haskell-side budget, Koios-confirmed, < the Rust 1,088,456). *** SIGNIFICANT: dugite matching aiken byte-for-byte at every UPLC layer is a STRONG correctness signal; the residual is the Rust-UPLC-ecosystem-wide builtin-mem-size accounting vs Haskell — a REAL consensus bug a NODE must fix (the chain runs Haskell), even though the aiken smart-contract tooling shares it. NEXT WAKE: DIAGNOSE the +68-word builtin — focused RAW-source comparison of dugite builtin/sized.rs (value→mem-size) vs Haskell plutus ExMemoryUsage instances (BuiltinList cons-cell, BuiltinPair, Data node, Integer 64-bit word-size) for the heavy builtins; find the single type whose size dugite/aiken over-count by 68 words; surgical fix in sized.rs gated so the 71 byte-exact controls + 999 UPLC conformance goldens stay green (no Haskell runtime needed — ExMemoryUsage is readable from raw plutus source). LESSON: the 3-wake #24 chain (diagnose→analyze→pin) systematically eliminated decoder/ScriptContext/cost-model/artifact via aiken-2nd-oracle + Python-TxInfo-reconstruction + Koios-pparams, isolating a subtle Rust-vs-Haskell mem-size accounting bug — the kind tests-green AND even the aiken oracle CANNOT catch (aiken agrees with the bug).
  *** wake492 (ultracode): DRIVE #24 ANALYZE (analyze Workflow wvdwf3vt0, in-turn ~27min/224K tok, aiken v1.1.22 2nd-oracle; all 4 temp patches REVERTED, tree clean). RULED OUT the obvious ADA-CS-28-zeros lead myself FIRST (HEAD-read: PlutusValue::to_data:391 correctly maps the [0u8;28] sentinel to empty b"" — #438 HEAD-verify discipline). *** TERM_vs_CEK SPLIT (conf 0.95): extracted dugite's APPLIED TERM (script·datum·redeemer·ScriptContext) for the over-costing Spend@3 of cluster 126af7b3 + ran `aiken uplc eval -c` → aiken mem=1,088,456 == dugite mem=1,088,456 (byte-IDENTICAL), both OVER on-chain declared mem=1,073,888 (the +14,568 cluster-A delta). CONTROL: Spend@2 (dugite=aiken=declared 831,789) + Mint@0 (1,256,320==declared) in the SAME tx byte-exact → dugite CEK+cost-model application CORRECT; the over-cost is f(dugite_term)≠f(haskell_term) = TERM-class, NOT a CEK bug. (aiken cpu not comparable — aiken bundles 16000/step vs on-chain V2 23000/step; mem is the version-stable oracle axis.) *** ScriptContext Data RULED OUT (the big elimination): dumped dugite's built ScriptContext Data + diffed NODE-BY-NODE vs an INDEPENDENT Python transTxInfoV2 reconstruction (raw tx body + raw resolved-UTxO CBOR) → byte-IDENTICAL across ALL 12 V2 TxInfo fields: txInfoInputs List[4] Set-TxIn-sorted (163 nodes, own-input last both), spent-output Address Constr0[ScriptCred, Just StakingHash ScriptCred] ✓, Value Map[3] ADA-empty-CS-first+sorted-policies ✓, inline datum ✓, txInfoOutputs 183 nodes ✓, refInputs List[5] ✓, redeemers Map[3] byte-identical, data Map[0] (witness-only, no inline inject) ✓. So the diagnose residual suspects (input ORDERING / Address-stake-cred / multiasset-Value / redeemers-ordering) ALL ELIMINATED (incl. a Minting-first redeemers re-sort experiment → cost UNCHANGED). *** PARADOX → NARROWED TO THE FLAT SCRIPT DECODER: identical ScriptContext + identical cost model yet +14,568 mem ⇒ the only non-re-derived component is dugite's flat decode of the 9955-byte ref-script (flat/decode.rs decode_term / program.rs from_flat) producing a term that reduces in ~145 MORE CEK steps than cardano-node's decode (+14568 mod 100 = 68 → machine-steps + a few extra Data-traversal builtin calls). Same validator f82adc2b over-costs by DIFFERENT deltas in different txs (+14568 vs +69303) → the delta tracks the tx CONTEXT the script folds, consistent with a decoded-term shape that costs more PER value-fold. conf 0.55 on the exact node (HONEST — could NOT pin a divergent Data node; the ScriptContext is provably identical). NEXT WAKE: the DECISIVE experiment — `aiken uplc decode --cbor` the ORIGINAL ref-script flat bytes + diff the UPLC term vs dugite's decode (flat/term.rs) + compare aiken-eval-of-original-flat budget to declared (1,073,888); if aiken-original==declared < dugite==1,088,456 → dugite's flat DECODER yields a costlier term = THE BUG (fix decode_term). SECONDARY to falsify first: re-pull the block pparams cost_models via koios.sh + confirm the dump cost_models_cbor matches the chain version. LESSON BANKED: aiken-as-2nd-oracle on the EXTRACTED applied term cleanly split term-vs-CEK, and a Python TxInfo reconstruction eliminated the ENTIRE ScriptContext — a powerful pinning method for phase-2 cost divergences.
  *** wake488 (ultracode): SCHEDULE #24, DRIVE re-DIAGNOSE at HEAD (diagnose Workflow w94vziez3, in-turn ~21min/169K tok, conf 0.86). Gathered the 51 budget-exhausted dumps (49 PV8 + 2 PV7, all Babbage V2, dugite Err where on-chain is_valid=true). *** REAL-vs-ARTIFACT RESOLVED = REAL: the wake326 "dumps omit ref-input UTxOs" caveat DOES NOT HOLD at HEAD (the dumps were re-captured complete) — decoded all 769 dumps tx inputs(0)/reference_inputs(18)/collateral(13) vs utxo_pairs → 769/769 UTxO-COMPLETE, 0 missing. So dugite builds the CORRECT ScriptContext; the over-cost is a GENUINE consensus divergence (49/51 over-cost spends to budget exhaustion where the chain ACCEPTS them → would break mainnet Babbage on popular DEX spend patterns). *** wake326 ROOT-CAUSE REFUTED (the #438 dont-trust-stale-conf lesson): NOT a fixed +4230mem/+1531582cpu, NOT "every inline-datum spend." Measured (instrumented full-budget per-redeemer run, since REVERTED — tree clean): deltas form DISCRETE RECURRING CLUSTERS, each FIXED-PER-OUTPUT-SHAPE + INDEPENDENT of validator bytecode AND datum size — 22x +5347552cpu/+14568mem, 16x +2012263/+5526, 13x +27990338/+69303, 4x +14724412/+50236; 69 Spend + 1 Mint divergent across 51 dumps, the OTHER 71 redeemers in the SAME txs BYTE-EXACT (incl other spends/mints), 1 Spend UNDER-costs (-10462456/-35924). The 22x cluster spans 3+ validators (f82adc2b/2467deab/19de6843) yet IDENTICAL delta + datum 351-453B → tied to the COMMON spent-output structure (addrtype=3 base-addr [script-payment+stake cred] + multiasset[2] value + inline datum), NOT validator/datum. HASKELL CROSS-REF (babbage.md §10/§143 vendored ref): transTxOutV2 inline datum = canonical structural Data (MemoBytes stripped) — dugite plutus_data_to_data/output_datum_to_plutus FAITHFUL; txInfoData = witness datums ONLY (dugite does NOT inject inline) — FAITHFUL; CEK cost model byte-exact (71 byte-exact redeemers prove it). RULED OUT: resolve_spend_datum (returns datum once, not in CEK loop), txInfoData injection, datum translation, builtin-cost-coefficient. RESIDUAL SUSPECTS (SPEND + output-shape-specific): base-address STAKE-credential Data wrapping, multiasset Value Data ordering/ADA-entry placement via the own-input lookup, or TxInInfo list ordering/dedup interacting with findOwnInput. NOT reclassified as artifact (REAL). DECISION: keep #24 OPEN, root-cause CORRECTED + re-scoped, conf 0.86; exact line NOT yet pinned (the weakest link — needs a 2nd-oracle per-builtin trace). NEXT WAKE: ANALYZE #24 — run cardano-node evaluateTransactionExecutionUnits OR aiken-uplc per-builtin/per-step trace on ONE cluster dump (126af7b3 Spend@3) + diff vs dugite to byte-pin the exact differing Data node; OR directly inspect dugite Value/Address Data translators vs Haskell transTxOutV2/transAddr for the base-addr-stake + multiasset[2] spent-output shape. how_to_confirm: phase2_repro over 769 dumps post-fix → 0 budget-exhausted + the 71 byte-exact STAY byte-exact (no regression). LESSON: the #481/#438 regenerate-and-re-validate discipline caught a stale conf-0.83 root-cause AND a stale dump-artifact caveat — BOTH wrong at HEAD.
  *** wake484 (ultracode): ran the #32 gauntlet (wsvwvp16v, 3 lenses, in-turn ~5.6min/238K tok) → PASSED 0/3 GOLD-STANDARD, every lens raw-source-reconfirmed (plutus d3c8d752 Builtins.hs + bddbf4b1 EvaluationContext.hs). (1) per-variant + for_script: Builtins.hs case A/B/D→consByteStringMeaning_V1 (lenient `BS.cons . fromIntegral`), C/E→_V2 (strict Word8); for_script V1/V2→A/B/D, V3→C/E; changPV=9/vanRossemPV=11; NON-MONOTONIC D-reverts-lenient at PV11 handled correctly (V1/V2@PV11→D→lenient NOT strict); rem_euclid(256)==fromIntegral incl negatives; Default=E (Haskell def=maxBound). (2) default-safety/conformance/no-regression: the SOLE production eval entry = eval_resolved_redeemer (computes for_script + threads intact); both feeders (parallel apply.rs:1040 / sequential validation/mod.rs:3627) source the LIVE ledger PV; the LATEST=E strict default is reachable ONLY from budget-free evaluate()/conformance-harness/#[cfg(test)] (ZERO production V1/V2 paths); conformance 999/999 (the 3 consByteString vectors stay strict); no-regression (all 0..=255 byte-identical; the +215 MATCH jump is purely out-of-range V1/V2 reject→accept flips). (3) threading + commit-safety: variant threaded end-to-end with NO mid-chain default; r.language+major_pv are real; the ONLY phase-2 entry is threaded; stage semantics.rs+6 files, exclude common.rs. COMMITTED 257658b360 (7 dugite-uplc files incl NEW builtin/semantics.rs, 1 crate; common.rs + proptest-regressions EXCLUDED via explicit staging). #32 DONE — the 215-dump LIVE Babbage-PV8 consensus divergence is CLOSED, byte-exact-proven (phase2_repro 215→0, 502→717 no-regression) + adversarially verified + conformance-preserved. *** NEW latent item surfaced by the gauntlet → FILED #33 [L]: WriteBits has a D/E-only maximumInputLength→builtinResultFailure check (variants A/B/C lack it) that dugite neither implements nor variant-gates — PRE-EXISTING, ORTHOGONAL to #32, NOT in the 215 PV8 divergences (bitwise builtins unavailable at Babbage). NEXT WAKE: SCHEDULE #24 [M] (PlutusV2 inline-datum-SPEND over-cost — ~51 budget-exhausted dumps, now the dominant remaining phase-2 divergence) → re-DIAGNOSE/pin the exact over-cost line.
  *** wake480 (ultracode): DRIVE #32 ROOT-CAUSED→FIXING+VERIFYING (fix Workflow wpvn26n23, in-turn ~16min/138K tok). Added SemanticsVariant{A-E} + for_script(language,major_pv) (NEW builtin/semantics.rs; V1/V2→A/B/D all lenient, V3→C/E strict; CHANG_PV=9 / VAN_ROSSEM_PV=11; LATEST=E strict default) + cons_byte_string_strict()=matches!(C|E). Threaded the Copy variant evaluate_with_budget→step→return_compute→apply/force_value→apply_builtin/force_builtin→denote (6 dugite-uplc files); computed ONCE in eval_resolved_redeemer (r.language+major_pv in scope; PUBLIC sig unchanged). consByteString arm branches: LENIENT (A/B/D) BigInt rem_euclid(256)→to_u8 (matches Haskell `BS.cons . fromIntegral` incl -1→0xFF / 256→0x00); STRICT (C/E) keeps bigint_to_u8. (Used .ok_or_else not .expect — crate denies clippy::expect_used; None unreachable.) replicateByte(:759)/appendString/equalsString/encodeUtf8 + cost-model selection (is_variant_b/apply_v1/v2/v3/resolve_applied_costs) UNTOUCHED (INDEPENDENTLY grep-verified). CONFORMANCE PRESERVED (the #1 risk): non-phase2 callers default to LATEST=E strict → the 3 consByteString conformance vectors keep strict-fail-on-out-of-range; INDEPENDENTLY re-ran fmt+clippy -p dugite-uplc -D warnings+nextest -p dugite-uplc --features conformance = 1463/1463 (999 vectors green). 8 new tests. Diff=1 crate (denotations/dispatch/mod/eval_redeemer/step/tests-conformance + NEW builtin/semantics.rs). *** VERIFYING BYTE-EXACT PROOF (rebuilt phase2_repro, re-ran the 769-dump batch — the OFFLINE byte-exact harness, NOT just tests-green): 717 MATCH / 52 DIVERGE (was 502 MATCH / 267 DIVERGE pre-fix); consByteString divergences 215→0 (target hit); MATCH jumped EXACTLY +215 (502→717) so the 502 prior-passing dumps are UNCHANGED (no regression); remaining 52 = 51 budget-exhausted (#24 over-cost) + 1 DIVERGE_accept. The 215-dump Babbage-PV8 consensus divergence is GONE — this IS the runbook byte-exact replay proof. NEXT WAKE: GAUNTLET #32 (3 lenses: (1) per-variant Haskell match + the strict⟺V3 / for_script mapping; (2) over-rejection-now-gone / conformance-preserved / replicateByte+cost untouched / 502-unchanged-no-regression; (3) commit-safety incl. git-add the new builtin/semantics.rs) → commit on pass. Fix UNCOMMITTED.
...
- PASSED 2026-06-10 (wake524, #26b V3 gov-action ScriptContext map ordering = ledger Script<Key, gauntlet wab7h0rjj): 0/3 refute, GOLD-STANDARD, raw-source-reconfirmed (cardano-ledger ebed62de + plutus 1.65.0.0). (1) per-field ledger-order: all 3 V3 GovernanceAction fields (TreasuryWithdrawals `Map AccountAddress Coin`, UpdateCommittee members_to_add `Map ColdCred Epoch`, members_to_remove `Set ColdCred`) built via transMap=unsafeFromList∘map∘Map.toList / Set.toList (TxInfo.hs L666-694) preserving the ledger Credential Ord Script<Key (ScriptHashObj first ctor); AccountAddress-Ord collapses to Credential-Ord (per-tx network dropped); plutus AssocMap ToData no-sort; dugite cmp_ledger (Script=0<Key=1 then hash) == ledger derived Ord; V3-only ⇒ #26-v1 Plutus-Key<Script trap inapplicable; tags unchanged (PubKey Constr0/Script Constr1). (2) scope: confined to gov_action_to_data; encode/governance.rs INERT (tx-id=blake2b256(raw_body_cbor) KeepRaw, encode forge-only); dedup safe (read_set_strict rejects byte-dups); no over-sort. (3) commit-safety: purely ordering, honest txs unaffected, latent-but-correct by-construction (#26/#29/#30 standard); stage populate_gov.rs only, exclude common.rs. COMMITTED 8685e95e56 (1 crate dugite-uplc). #26b DONE — closes the entire #26-excluded gov-map-ordering scope; TreasuryWithdrawals reuses the #26-gauntlet-proven ledger_ordered_withdrawals helper.
- PASSED 2026-06-10 (wake512, #33 WriteBits maximumInputLength D/E cap, gauntlet wzwkl05yw): 0/3 refute, GOLD-STANDARD, raw-source-reconfirmed (plutus 0c3d72dc). (1) ensurable/constant/order byte-exact: maximumInputLength=4096 (NOT 8192=output-cap); gate ensurable=matches!(D|E) (A/B/C no cap); strict `>` on the INPUT bytestring; guard BEFORE the per-index check (over-len fails even with empty/in-range indices); builtinResultFailure==builtin_failure. (2) writeBits-ONLY: no maximumInputLength cap on any other bitwise builtin in Haskell (their D/E diff = CByteString costing only); dugite capped none of them; replicateByte/integerToByteString (8192) untouched; variant C NOT capped (>4096 Ok under C); conformance 1470/1470; normal <=4096 inputs byte-identical. (3) commit-safety: purely additive 163/0; honest scripts identical; only rejects what Haskell rejects; LATENT (bitwise builtins absent pre-PV10/V3 → zero live impact, defense-in-depth for V3@PV11+ [E] + V1/V2@PV11+ [D]); stage semantics.rs+denotations.rs, exclude common.rs. COMMITTED b64cbfa15e (1 crate dugite-uplc). #33 DONE — closes the BuiltinSemanticsVariant completeness item surfaced by the #32 gauntlet.
- PASSED 2026-06-09 (wake484, #32 consByteString BuiltinSemanticsVariant, gauntlet wsvwvp16v): 0/3 refute, GOLD-STANDARD, raw-source-reconfirmed (plutus d3c8d752/bddbf4b1). (1) per-variant byte-exact: A/B/D→lenient (BS.cons . fromIntegral mod-256), C/E→strict Word8; for_script V1/V2→A/B/D / V3→C/E (changPV=9/vanRossemPV=11); NON-MONOTONIC D-reverts-lenient handled; rem_euclid==fromIntegral. (2) strict default (LATEST=E) reachable ONLY from test/conformance callers — sole production entry eval_resolved_redeemer threads the real for_script variant; conformance 999/999; no-regression (0..=255 byte-identical, +215 out-of-range flips). (3) threading end-to-end no mid-chain default; ONLY phase-2 entry threaded. COMMITTED 257658b360 (1 crate dugite-uplc incl new semantics.rs). VERIFYING (banked wake480): phase2_repro 215→0, 502→717 MATCH no-regression. Surfaced #33 (WriteBits D/E maximumInputLength, latent/orthogonal).
...
- 2026-06-10T08:00Z wake524 ~ #26b GAUNTLET→DONE: gauntlet Workflow wab7h0rjj (3 lenses, in-turn ~3.6min/209K tok) PASSED 0/3 GOLD-STANDARD (raw-source cardano-ledger ebed62de + plutus 1.65.0.0: all 3 V3 gov-action fields ledger Script<Key via transMap=Map.toList+Set.toList + AssocMap no-sort; V3-only #26-v1-trap inapplicable; encode/governance.rs INERT; dedup safe; purely-ordering). COMMITTED 8685e95e56 (1 crate dugite-uplc, populate_gov.rs ONLY; common.rs EXCLUDED). #26b DONE — #26-excluded gov-map-ordering scope CLOSED.
- 2026-06-10T07:00Z wake520 ~ #26b ROOT-CAUSED→FIXING: fix Workflow w79kt37ey (in-turn ~5min/85K tok). 3 ledger-order sorts in populate_gov.rs gov_action_to_data (TreasuryWithdrawals reuses #26-proven ledger_ordered_withdrawals; members_to_add/members_to_remove sort_by cmp_ledger + dedup). 1 crate dugite-uplc +292/-26; 6 new tests; INDEPENDENTLY re-verified fmt+clippy -D warnings+nextest 469/469. Uncommitted; gauntlet next.
- 2026-06-10T06:00Z wake516 ~ #26b NEW→ROOT-CAUSED: diagnose Workflow w7bg9vcwg (in-turn ~5.2min/115K tok, conf 0.93, raw-source cardano-ledger ebed62de + plutus 1.65.0.0). All 3 V3 gov-action ScriptContext fields (TreasuryWithdrawals/members_to_add maps + members_to_remove list) = LEDGER Script<Key (transMap=Map.toList preserves ledger Cred Ord; Set.toList) — dugite produces Key<Script → DIVERGES; encode/governance.rs INERT (raw-wire preserved). Fix = sort 3 fields by Credential::cmp_ledger in populate_gov.rs (1 crate). Next FIXING.
- 2026-06-10T05:00Z wake512 ~ #33 GAUNTLET→DONE: gauntlet Workflow wzwkl05yw (opus 3-lens panel, in-turn) PASSED 0/3 GOLD-STANDARD (raw-source plutus 0c3d72dc: 4096-constant / ensurable=matches!(D|E) / strict-`>` / guard-before-index byte-exact; writeBits-ONLY; variant C not over-strict; conformance 1470/1470; purely additive 163/0; latent). COMMITTED b64cbfa15e (1 crate dugite-uplc: semantics.rs + denotations.rs; common.rs ep246 scratch + proptest-regressions EXCLUDED). #33 DONE.
...
- 2026-06-09T21:00Z wake480 ~ #32 ROOT-CAUSED→FIXING+VERIFYING: fix Workflow wpvn26n23 (in-turn ~16min/138K tok) SemanticsVariant threaded into denotation, consByteString lenient rem_euclid-256 for V1/V2; replicateByte/cost untouched; conformance 999/999 + nextest 1463/1463 INDEPENDENTLY verified. VERIFYING BYTE-EXACT: phase2_repro 769 dumps → consByteString 215→0, 502→717 MATCH (no regression), 52 remain (#24). Uncommitted; gauntlet next.
- 2026-06-09T22:00Z wake484 ~ #32 gauntlet wsvwvp16v (3 lenses, in-turn ~5.6min/238K tok): PASSED 0/3 GOLD-STANDARD (raw-source plutus d3c8d752/bddbf4b1: per-variant byte-exact incl non-monotonic D-reverts-lenient; strict-default reachable only from test/conformance; threading end-to-end; conformance 999/999; no-regression). COMMITTED 257658b360 (1 crate incl new semantics.rs). #32 DONE — 215-dump live Babbage-PV8 consensus divergence CLOSED. Filed #33 (WriteBits latent). Next #24.
- 2026-06-09T23:00Z wake488 ~ #24 re-DIAGNOSE at HEAD: diagnose w94vziez3 (in-turn ~21min/169K tok, conf 0.86). REAL not artifact (769/769 dumps UTxO-COMPLETE); wake326 root-cause REFUTED (NOT fixed-delta, NOT every-inline-spend) — re-scoped to SPEND ScriptContext over-cost in fixed-per-output-shape clusters (base-addr-stake + multiasset[2]), validator/datum-independent, 69 Spend+1 Mint divergent / 71 byte-exact. Translators+cost-model faithful → bug in SPEND ScriptContext build. Next ANALYZE: 2nd-oracle per-builtin trace to pin the exact Data node.
- 2026-06-10T00:00Z wake492 ~ #24 ANALYZE: analyze wvdwf3vt0 (in-turn ~27min/224K tok, aiken 2nd-oracle). TERM-level NOT CEK (aiken==dugite mem 1,088,456 > declared 1,073,888 on dugite applied term; Spend@2/Mint@0 byte-exact). ScriptContext Data RULED OUT (node-by-node == Python transTxInfoV2, all 12 fields). NARROWED to the FLAT SCRIPT DECODER (decode_term/from_flat) yielding a ~145-extra-CEK-step term vs cardano-node. conf 0.95-term/0.55-node. Next: decisive aiken-decode-original-flat vs dugite experiment.
- 2026-06-10T01:00Z wake496 ~ #24 PIN experiment: analyze wb86tiwa7 (in-turn ~27min/221K tok, aiken 2nd-oracle). FLAT DECODER RULED OUT (aiken decode==dugite decode byte-identical); 3 Rust CEKs all give 1,088,456 vs Haskell-on-chain 1,073,888; cost model Koios-confirmed correct (ep81 0/175). PINNED CAUSE=(C) builtin-result-mem-SIZE over-count SHARED by the Rust UPLC family (dugite+aiken) vs Haskell — +68-word residual on one list/pair/Data builtin. dugite==aiken at every layer (strong correctness signal). conf 0.85. Next DIAGNOSE: sized.rs vs Haskell ExMemoryUsage for the +68-word type.
- 2026-06-10T02:00Z wake500 ~ #24 mem/step DIAGNOSE: w0jsp1jbm (in-turn ~10min/172K tok, conf 0.8) → verdict BLOCKED. Exact reconcile: +14,568 mem = +145 CEK steps + 68-mem extra builtin calls; RAW-source RULED OUT step-charging (==Cek/Internal.hs), ExMemoryUsage (==ExMemoryUsage.hs), mem-model-shape (all heavy builtins Constant in both). The +145 steps = the term REDUCES 145 more times under the Rust family (dugite==aiken) than Haskell — a reduction-TRACE difference needing a Haskell uplc step-trace (ghc/cabal absent). PARKED #24 BLOCKED-on-Haskell-oracle. Next #33.
- 2026-06-10T03:00Z wake504 ~ #33 NEW→ROOT-CAUSED: diagnose w3r7v380b (in-turn ~4min/77K tok, conf 0.9, raw-source plutus 0c3d72dc). writeBits maximumInputLength=4096-byte INPUT cap gated `ensurable`=matches!(D|E) (Builtins.hs:2725); ONLY writeBits (others = CByteString costing only; replicateByte/intToBs already 8192-cap). Fix=semantics.rs bitwise_max_input_enforced() + writeBits-arm guard before the index loop; variant already threaded. Next FIXING.
- 2026-06-10T04:00Z wake508 ~ #33 ROOT-CAUSED→FIXING: fix Workflow wblte27aw (in-turn ~7.5min/72K tok) bitwise_max_input_enforced()=matches!(D|E) + writeBits 4096-byte INPUT cap before the index loop; writeBits-ONLY (scope guard verified, purely additive); 7 tests; INDEPENDENTLY re-verified fmt+clippy+nextest+conformance 1470/1470. Uncommitted; gauntlet next.

## Last node state
- sampled: 2026-06-10T08:00Z (wake524)  no dugite-node running (node_pids="") — #26b phase-2/governance gauntlet+commit (offline); free_disk ~146GB, free_ram ~1GB, halt=false.
- sampled: 2026-06-10T07:00Z (wake520)  no dugite-node running (node_pids="") — #26b phase-2/governance fix code/test-only (offline); free_disk ~149GB, free_ram ~1GB, halt=false.
- sampled: 2026-06-10T06:00Z (wake516)  no dugite-node running (node_pids="") — #26b phase-2/governance diagnose (offline raw-source); free_disk ~149GB, free_ram ~1GB, halt=false.
- sampled: 2026-06-10T05:00Z (wake512)  no dugite-node running (node_pids="") — #33 phase-2/uplc gauntlet+commit (offline); free_disk ~149GB, free_ram ~1GB, halt=false.
- sampled: 2026-06-10T04:00Z (wake508)  no dugite-node running (node_pids="") — #33 phase-2/uplc fix (offline); free_disk ~177GB, free_ram ~4GB, halt=false.
- sampled: 2026-06-10T03:00Z (wake504)  no dugite-node running (node_pids="") — #33 phase-2/uplc diagnose (offline raw-source); free_disk ~177GB, free_ram ~4GB, halt=false.
- sampled: 2026-06-10T02:00Z (wake500)  no dugite-node running (node_pids="") — #24 phase-2/uplc mem-step diagnose (offline source-compare); free_disk ~177GB, free_ram ~4GB, halt=false.
- sampled: 2026-06-10T01:00Z (wake496)  no dugite-node running (node_pids="") — #24 phase-2/uplc pin experiment (offline aiken decode/eval + Koios pparams); free_disk ~177GB, free_ram ~4GB, halt=false.
- sampled: 2026-06-10T00:00Z (wake492)  no dugite-node running (node_pids="") — #24 phase-2/uplc analyze (offline aiken 2nd-oracle + applied-term extraction); free_disk ~177GB, free_ram ~4GB, halt=false.
- sampled: 2026-06-09T23:00Z (wake488)  no dugite-node running (node_pids="") — #24 phase-2/uplc re-diagnose (offline phase2_repro + dump-decode); free_disk ~177GB, free_ram ~4GB, halt=false.
- sampled: 2026-06-09T22:00Z (wake484)  no dugite-node running (node_pids="") — #32 phase-2/uplc gauntlet (offline); free_disk ~177GB, free_ram ~4GB, halt=false.
- sampled: 2026-06-09T21:00Z (wake480)  no dugite-node running (node_pids="") — #32 phase-2/uplc fix+phase2_repro verify (offline); free_disk ~177GB, free_ram ~4GB, halt=false.
