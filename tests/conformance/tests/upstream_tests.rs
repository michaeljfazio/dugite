...
// ── Per-vector tests (auto-generated by build.rs) ────────────────────────────
//
// One `#[test]` per leaf fixture across every upstream area.  Filter by
// area with nextest expression syntax, e.g.
//   `cargo nextest run -p dugite-conformance --features upstream-conformance \
//       --test upstream_tests -E 'test(/^upstream_cardano_ledger_/)'`
//
// The umbrella `*_golden_decodes` / `*_checks` tests above remain as a
// quick all-in-one sanity gate; the per-vector tests are the granular
// pass/fail coverage signal.
include!(concat!(env!("OUT_DIR"), "/generated_upstream_tests.rs"));
